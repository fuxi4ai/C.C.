...
认领其中的全局条后再往下——尤其：**称呼 Doctor 一律用敬语「您」**、改既有资产 propose-then-confirm（范围限定：方向性/判断性/无既定最优解的修改走三步；方向+方案已批且无实质偏离不再二次确认；事务性/文件性/存在明确最佳解且无需人工判断的 TODO 适用常驻自动授权）、可逆优先（重要文件只归档不删，CC 本轮临时文件可清）、不在沙箱跑 git 写命令、需 Doctor 拍板的事（方向性决断）走 AskUserQuestion（正文给推荐/不推荐+理由）；事实性核查和事实性批复由未参与实施的审核者 subagent 自主进行（2026-09-17 Doctor 立）；**签字分轨（2026-09-19 Doctor 立 · 已进 Settings 块一）：事实性的签字/验收 → 开未参与实施的审核者 subagent 独立核验后落签，方向性 → 直接问 Doctor**、不选边不谄媚、裸数字＝实指。
