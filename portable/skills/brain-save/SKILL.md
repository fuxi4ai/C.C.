...
> **⚠ 同族第 3 次复发 → 加硬规：`git add` 清单必须【逐文件】给出，禁目录路径（2026-09-26 立）**
> **触发**：2026-09-26 /save 场，CC 给的清单里写了**目录**——`git add .skills/QA` 与 `git add rules/`。ignore 判据**确实过了一遍，但只核了「列出来的那些路径」**；目录**展开出来的文件一个都没核** ⇒ 4 个 `.skills/QA/*.bak_*`（brain）与 2 个 `rules/*.bak_20260926_pre_*`（行业研究）被扫进版本，事后靠 `git rm --cached` 清出。
