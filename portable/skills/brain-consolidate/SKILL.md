...
- **v1.3.3**（2026-09-19 · 措辞订正 · Doctor 批「改并四端重发」）：把 v1.3/v1.3.1 引入的 **8 处「腿」字术语**（`三条腿全覆盖` / `哪条腿冲突` / `形态（含哪条腿）` / `任一腿` / `任一条腿` / `第三腿`）统一改为「**对读线**」。依据＝Doctor **2026-08-30 立「禁『腿』字术语」**（不用「腿」形容管线/通道），而本文件系该禁令**之后**新写、当时未受其约束。**选「对读线」而非「对读面」的理由**：「核对面」在本文件已指 ①/②/③ **三层**，而此处要指的是三层之间的**三条配对连线**（①↔②/②↔③/①↔③），两者必须可区分。**纯措辞改动，规则语义零变**；`description` 同步改，故下一步 fresh-session 注入文本会随之一并换代。
