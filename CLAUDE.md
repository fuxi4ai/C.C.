...
重大改动遵循 **propose-then-confirm**：先提案 → 出具体改动清单 → Doctor 批准 → 才执行（"批准做某事" ≠ "批准具体怎么改"）。**范围限定**：方向性/判断性/无既定最优解的修改走三步确认；事务性/文件性/存在明确最佳解且无需人工判断的 TODO 适用常驻自动授权（2026-08-17 立）。完整见 [[Doctor协作偏好]]。
...
**边界（2026-08-17 VV 六轮立 · 七轮整合）**：GOTCHAS 自动登记属于**预授权治理留痕**——不授权修复被审资产、验收签字、提交或发布；新条目状态只能 `🔄 待修复/已修待验` / `⚠️ 已知风险`，不得由实施者自行标 `✅`（事务性项可由未参与实施的 subagent 独立审核后代落，2026-08-29 常设授权）；同根复发优先追记既有条目，避免重复造条。
...
**状态**: 🔄 待修复/已修待验 / ⚠️ 已知风险（✅ 仅由 Doctor 或指定独立验收方落，实施者不得自标）
**优先级**: 🔴 高 / 🟡 中 / 🟢 低
**触发场景**: 
**硬证据/最小复现**: 
**根因**: 
