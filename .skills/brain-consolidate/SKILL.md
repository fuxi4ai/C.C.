...
**判据与处置（分级 · 不自行择一）**。**分级按「哪条对读线冲突 + 能否判定」两个轴定**；为避免与上表「层危险度」的符号撞义，**本表不用 🔴🟡 符号、用文字级名**：
...
| **同错** ⚠ | **①/② 彼此一致、却与 ③ 冲突**（或 ①↔③、②↔③ 任一对读线） | **最危险形态**：两个「常用层」互相印证、只有少读的 canonical 是对的 → **优先级最高，逐条列出 ①②③ 三方原文**，不得只报两方 |

> **三条对读线全覆盖**：①↔② / ②↔③ / ①↔③ **任一对读线**的冲突都按上表定级，**①↔③ 与 ②↔③ 同权**——不要因为 ③「平时不读」就降级。
...
- **v1.3**(2026-09-19 · Doctor 立「给 consolidate 加记忆冲突核查机制，执行时需审核者 subagent 审核」)：**新增 Step 1.5 记忆冲突核查**——
  ...
  3. **冲突分级**（级名 **一致 / 陈旧 / 对撞 / 同错·⚠** · 按「哪条对读线冲突 + 能否判定」两轴定）：语义一致不算 · 单侧陈旧（有权威源可判）列 diff 改落后方 · 对撞（两边都有出处）**不自行择一**、单列待 Doctor 裁 · **同错⚠（①② 彼此一致却与 ③ 冲突）优先级最高、逐条列三方原文**。**三条对读线 ①↔②/②↔③/①↔③ 全覆盖、同权。**
...
- **v1.3.1**（2026-09-19 · **v1.3 独立复验后修订** · 复验结论 PASS_WITH_LIMITS · 11 项缺陷）：复验由**未参与实施**的 subagent 执行（四端逐字节对拍 + 内容审查 + 影响面复查 + 漏改核查 + 自证扫描面完整性）。**已修 11 项**——
  1. **⚠ 级定义与来源案例不符**（中）：原写「①②层之间任何冲突」，而本案实况是 **①② 同错、只有 ③ 对**；且第三对读线 ①↔③ 无归级 → 级名改「**同错 ⚠**」并明写该形态，三条对读线全覆盖同权。
...
- **v1.3.3**（2026-09-19 · 措辞订正 · Doctor 批「改并四端重发」）：把 v1.3/v1.3.1 引入的 **10 处「腿」字术语**（**7 种词形**：`三条腿全覆盖`×3 · `哪条腿冲突`×2 · 裸 `三条腿` · `含哪条腿` · `任一腿` · `任一条腿` · `第三腿`）统一改为「**对读线**」。依据＝Doctor **2026-08-30 立「禁『腿』字术语」**（不用「腿」形容管线/通道），而本文件系该禁令**之后**新写、当时未受其约束。**选「对读线」而非「对读面」的理由**：「核对面」在本文件已指 ①/②/③ **三层**，而此处要指的是三层之间的**三条配对连线**（①↔②/②↔③/①↔③），两者必须可区分。**纯措辞改动，规则语义零变**；`description` 同步改，故下一步 fresh-session 注入文本会随之一并换代。
  **同轮独立复验**（两轮 · 均由未参与实施的 subagent 执行 · 实读四端 + 只读解开 `.git` 松散对象取提交件做全量 diff）。
  - **第一轮判 PASS_WITH_LIMITS**，查出本行原写「**8 处**」系**计数错误**（当时数的是 Edit 次数、不是出现次数；实为 10 处 / 7 种词形，且词形清单漏列裸 `三条腿`）——已当场订正；第二轮逐项复算确认：总数 10、词形 7 种、各词形出现次数（`三条腿全覆盖`×3 · `哪条腿冲突`×2 · 裸 `三条腿`/`含哪条腿`/`任一腿`/`任一条腿`/`第三腿` 各×1）全部吻合，且这 10 处在 v1.3.1 输出件中已存在（v1.3.2 未增未减），归属表述无误。
  - **第二轮判 PASS_WITH_LIMITS**，又指出本段自身三处**「记录口径与盘面脱节」**（与本行被订正的那处同病）：① 原文「改动仅 7 行改 + 1 行增、无任何规则条文增删改」**字面不成立**——那 7 行里 6 行正是规则区正文；② 「自建 4 负例」的四种失败形态与官方 9 例中的四例**重合**，写成「官方 9 例 + 自建 4 负例」会被读成 13 例独立覆盖；③ 「对读线 12 处 / 核对面 4 处」是**修正前**被测件的数，装船版本已不同。**三处均已按第二轮口径改写（见下）。**
  - **已独立复现通过的项**：仓内三端 SHA 逐字节一致（`cmp` 退出 0）· runtime 与 canonical **仅差末端 1 个 `0x0a`**（`cmp -l` 无任何差异字节偏移）· 四端 `yaml.safe_load` 全过且 runtime **无**双 frontmatter · 6 行规则区改动**逐字符只差「腿」→「对读线」的术语替换**（机械替换检验 6/6 True），无规则语义增删改 · 残留「腿」全部落在版本历史与复验段的**引用留痕**内、正文规则区零残留 · 「对读线」（配对连线）与「核对面」（①/②/③ 三层）两词分工未混用（逐处核，**具体处数随文本修订变动，故本处不记数**）· 包内无杂物、`unzip -t` 无错、条目结构与同族同形 · `.gitignore` 不挡三端且三端均已在版本内 · 对拍器官方 **9 例 rc 0**（复验方另自建 4 负例独立复现其报错路径，**形态与官方四例重合、非增量覆盖**）· 复验全程只读，跑后四端 SHA 与仓况逐位未变。
  - **残留未核**：Mac 侧其他安装副本（Claude-3p 侧 / plugin cache / Gateway store）沙箱不可达，是否同版未核。
