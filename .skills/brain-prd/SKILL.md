---
name: "brain-prd"
description: "立 PRD——功能/需求验收基线，不是审批单。任务已获执行授权且无阻断性歧义时直接立卷、并在既有授权范围内实施；只有具体方向、范围或需求存在阻断性歧义时才就该分歧请 Doctor 裁定。触发：用户说 `/prd` 或 `/prd [任务简称]` 或「立PRD」「起草PRD」「写交付标准」「写个go」；或 CC 判断任务达复杂度阈值（多文件改/跨小时/跨项目/凌晨自主）且存在功能或明确非功能需求时主动立卷。对齐为条件触发（需求模糊/信息不足/范围冲突/验收标准无法落成时才问）。§二 验收主体＝功能/需求（用户可感知结果＋明确非功能需求），文件/Grep/脚本输出只是证据，过程项进 §2.5 执行清单（task_status）。无功能交付的事务不立 PRD 走客观 TODO。落 `brain/logs/checkpoints/{date}_{任务}_PRD.md`。严守 G-X4：PRD checkbox 实施者只填 [?]/[!]/[~]+证据，[✓] 由 Doctor 或未参与实施的指定独立验收方落、实施者不自签、永不自动关闭、不在沙箱跑 git。"
---
...
**不用**（避免官僚化）：单一问答 / 解释 / 研究讨论；单文件小改（< 10 处）；**无功能交付的事务**——纯清洗/去重/迁移/归档等事实核验类走客观 TODO（G-X136 常驻授权代勾）。**规模（多文件/跨小时）只是必要条件，「有功能可验收」才是立 PRD 的判据**（2026-08-15 Doctor 立 · 回归「功能验收」初衷）。
...
### Step 1 · 判定要不要立
按 §1。任务已获执行授权且无阻断性歧义 → 直接立卷（不问「是否同意立 PRD」）；授权无法核实 → 停在 draft 先核实授权；Doctor 明示「不用立 PRD」→ 不立，走客观 TODO。
