...
<!-- 2026-09-14 错题本积压复盘（Doctor「审查并修复」批全案 · 未闭环 11 条=已修待验 9 + 真待修复 1 + ⚠️ 1）：
     ...
     消化后计数：🔄 待修复 = 1（ERR-20260911-001）· ⚠️ = 1（ERR-20260830-001）· ✅ 2026-09-14 晚 Doctor 批「批准收取」→ 9 条已落签（CC 代记）。 -->
...
**状态**: ✅ 已验收（Doctor 2026-09-14 落签「批准收取」 · CC 代记 · 2026-08-20 实施完成 · 待 Doctor 或指定独立验收方验；实施者不自签 ✅ · 2026-09-14 复盘：gen L1040「不得升维成板块名单」在盘 ✓ · ✅ 已落签）
...
**状态**: ✅ 已验收（Doctor 2026-09-14 落签「批准收取」 · CC 代记 · 2026-08-26 句芒档1 修复完成 · 待 Doctor 或指定独立验收方验；实施者不自签 ✅ · 2026-09-14 复盘：dim1/dim3 按 date 同日重复=0 ✓ · ✅ 已落签）
...
**状态**: ✅ 已验收（Doctor 2026-09-14 落签「批准收取」 · CC 代记 · **2026-09-01 Doctor 裁「根治」→ CC 实施三层防线**：① 顶部 `sys.path.insert(0, 项目根)` bootstrap——保证班域内 import config 成功、`config.MARKET_DB` 随 `ZZJY_DATABASE_ROOT` 指向 /tmp 副本根（主因消除）；② `_db_path()` 删 `except Exception` 静默 fallback——不再回退直写 live（2026-09-14 复盘：bootstrap 三防线实读在盘 L22-26 ✓ · ✅ 已落签）fail-loud）；③ 写连接改走 `config.connect_write` 中央护栏——沙箱挂载盘（含 /sessions//mnt/）直写被拒（G019 兜底）。**全 scripts/ 扫查：唯本脚本缺 bootstrap**（其余 15 件均有）——四次复发同源收敛单文件。验证：py_compile ✓ · 班域模拟 `ZZJY_DATABASE_ROOT=/tmp/zzdbroot` → `_db_path=/tmp/zzdbroot/Market-Data/market_data.db` ✓ · 负向无 env 沙箱域 → connect_write 抛 RuntimeError 拒绝 live ✓。**实施者不自签**——待独立验收）
...
**状态**: ✅ 已验收（Doctor 2026-09-14 落签「批准收取」 · CC 代记 · 2026-08-28 Doctor 裁「另立 code」（推荐项）→ 已改：`fetch_intl_index.py` SA 写入映射 `SA_WRITE_CODE={"NASDAQ":"NASDAQ_QQQ"}`（2026-09-14 复盘：L160 映射+L218 调用实读在盘 ✓ · ✅ 已落签） + `gen_daily_report.py` 两处 QQQ fallback（主路 ^NDX 缺当日行时顶替并标注「主路^NDX缺·应急代理」）· 自验：py_compile 两文件 ✓ · /tmp 库副本跑真实 stockanalysis 分支 → 20260827 两列并存（NASDAQ=^NDX 29641.56 保留 + NASDAQ_QQQ=QQQ 530.5 独立写入）✓ · 实施者不自签）
...
**状态**: ✅ 已验收（Doctor 2026-09-14 落签「批准收取」 · CC 代记 · 2026-09-01 Doctor 裁「换指数真身」→ 实施：`scripts/fetch_index_daily.py` 新建（tushare index_daily 000300.SH → `cn_index_daily` 表）+ 内嵌进 `fetch_theme_etf.py`（同批原子拉取·班 prompt 零改动）+ `gen_daily_report.py` 快照/大盘曲线改读指数表（`IDX`）（2026-09-14 复盘：cn_index_daily 表 655 行在库 ✓ · ✅ 已落签）+ bench_note「沪深300指数」+ `pct_span` .1f→.2f。验证：负向（无表→「待取数」fail-closed·exit 0）+ 正向（/tmp 镜像灌真实值→快照 -0.30%·产物零「代理/510300」残留）。**实施者不自签**——待独立验收；回填与重生成命令已贴 Doctor 终端）
...
**状态**: ✅ 已验收（Doctor 2026-09-14 落签「批准收取」 · CC 代记 · 2026-09-07 句芒档1 当场补标修复完成 · 待 Doctor 或指定独立验收方验；实施者不自签 ✅ · 2026-09-14 复盘：09-07 九行 viewpoint_owner 空=0 ✓ · ✅ 已落签）
...
**根因**: 回写工具存在、不在任何调度链（2026-07-28 全金融审查 P0-3 已立条挂账，两月未动）；信号池 3089→3986 行持续增长，欠账同步扩大。

**影响面**: 体系「记预测」密度高、「记结果」几乎为零——日报所引胜率（如一段确认 3d 84.5%）仍为 2026-07-06 静态快照，不随新数据滚动；「信号灵不灵」无法用库内数据回答。同族：predictor_accuracy 停更、board_snapshots 关键列空洞、龙鱼评分无收益闭环——**应升格通用教训候选**「预测/结果记账必须成对入调度」。

**正确做法**: ① 回写脚本入日链（信号 t+3/t+10 后自动补 excess 列——行情在 market_data.db，可离线补算存量 3986 行一步到位）；② 信号胜率引用改「滚动窗口+样本数+日期」动态口径，废除静态快照。

**来源**: 2026-09-11 全金融审计（recap.db 只读探针实跑 · 07-28 审计报告对照）
**追记（2026-09-11 晚间 · VV 交叉复核补充 · CC 接受）**: 「正确做法」①修正——回写**按持有窗到期**计算（信号日起 t+3/5/10 到点即写，**不论 resolve 与否**），不只统计已 resolved 信号，否则会漏掉失败、未兑现和无法成交的样本（完成偏误）；②补充分簇——同一事件跨公司/跨窗口要分簇，3986 行不能当 3986 个独立样本；③回补旧结果标 retrospective/diagnostic，只有从现在起冻结的才有资格逐步形成前瞻证据。

## [ERR-20260911-002] IPO 事件表无新行被算成「0 募资=平静」——覆盖判断缺位，双消费端同根（2026-09-11 VV 独立审计发现 · CC 实读产物确认）

**状态**: ✅ 已验收（Doctor 2026-09-14 落签「批准收取」 · CC 代记 · 2026-09-11 Doctor 批「三项全开工」· CC 实施双端修复 · 实施者不自签 ✅ · 2026-09-14 复盘：ipo_coverage 覆盖 20260817-20260915 complete=1 ✓ · ✅ 已落签）
...
**现象**: ipo_daily 最新事件日 20260722（7 周无新行）；09-11 风险快照仍显示「近 10 日募资 0 · 温度 0 · 平静」、复合风险文字「今日无募资（剂量 0=历史最安全态之一）」；09-10 烛照日报 F4 同款「平静 · 近 10 日新股 0 只 · 募资 0 亿」。CC 实读风险日报 HTML：卡片虽有「ipo数据至20260722」小字注记，但温度/触发语义未降级。

**根因**: 读取层窗口内无行时 `COALESCE(SUM(...),0)`（gen_daily_report.py F4 段 / build_risk_daily.py 复合卡）——「事件表无新行」被等同于「确认无事件」；而事件表无行既可能是无事件也可能是采集未覆盖。Market-Data `_health.json` overall=ok 未含 IPO 覆盖检查。

**影响面**: 两个日报把「不可判」呈现为「安全」；F4/复合风险卡的温度与文字都受影响。
...
**追记（2026-09-12 · VV 第三轮复验后修复 · 状态 🔄 已修待验·第四轮）**: 判断合同补全——① **事件版本绑定**：覆盖行加 `events_hash`（[scan_start,scan_end] 内事件行状态哈希），消费端按 `fetched_at` 最新行取证明并重算比对——不匹配=证明过期 → 不可判（新事件不得借旧证明；VV case19 反例已复刻通过）；② **NaN/Infinity 治理**：fetch 聚合前 `math.isfinite` 校验，非有限值计缺失不入合计；③ **行级缺失**：ipo_daily 加 `funds_missing` 列（当日缺失条数），消费端按窗口求和——旧行无标记/funds NULL → 缺失数未知 → 不可评（fail-closed）；④ **真零可评**：完整覆盖+空窗口 → funds_win=0 合法 quiet；⑤ **首屏三态**：有 pending 时「已知项未触发·部分不可评」，两项均 quiet 才称平静；⑥ **窗口/分母一致**：risk_daily 窗口改 `> cutoff`（不含 D-10）且 amt 分母加 `trade_date<=bj_today` 截止（未来成交额不稀释 ratio）；⑦ 缺 ipo_date 坏行 → complete=0。**判定式复验**：版本绑定失效/窄区间不可判/NaN/Inf/真零/旧行未知/坏行——七态全过 + py_compile 3 文件 ✓。**边界**：真实 API 完整性未核；旧 996 行无 funds_missing 标记 → 回填前窗口含旧事件行将显示不可评（fail-closed 预期）。

**2026-09-12 VV 现役源码隔离复验（同根补充，金额完整性尚有缺口；待 Doctor 批修）**：以本次 `fetch_ipo.py` 快照执行完整 fetch+ensure_table，接口替换为合成行，数据库仅为内存 SQLite。有限值/None/不可转换三正负控符合当前合同；NaN 落为 SQL NULL、Infinity 落为 inf，两例却仍 `funds_missing=0`。`complete=1` 表示接口覆盖，本轮不把这个字段本身定为错误；缺口是金额有效性被误报完整。转换后及汇总后均应检查 `math.isfinite`（防累加溢出），异常计缺失并向消费端保留 unknown/已知下限。未调用真实接口、未查询生产库，不能据此认定现库已有非有限金额。证据：`/Users/lunarabbit/Documents/AI4ME/Financial-Audit-outputs/harness-rollout/五项目注册与自检_20260912/business-replays.json` ipo_nonfinite；独立 Seatbelt 重放输出一致。代码、生产表及消费者均未修改，修复状态不得据机制复验升级为已修。
...
**状态**: ✅ 已验收（Doctor 2026-09-14 落签「批准收取」 · CC 代记 · 2026-09-14 CC 修复：plist 归位 + README 三班登记 · 实施者不自签 ✅ · 09-14 复盘：plist SHA 双端一致 ✓ · ✅ 已落签）
...
**状态**: ✅ 已验收（Doctor 2026-09-14 落签「批准收取」 · CC 代记 · 2026-09-14 Doctor 裁「话术分诊+阈值分级」· CC 实施+当日重推：yuantu_client.kg_health_meta + sync 落 yuantu_kg_health 表 + gen 分诊横幅 · 正负向测试全绿 · live recap.db 已带新表（备份 recap.db.bak_20260914_predaily）· zhuzhao-jiuyin-daily 已重推、Gateway 回读黄横幅命中 · 实施者不自签 ✅ · ✅ 2026-09-14 Doctor 落签「批准收取」 · CC 代记 · 目验归 Doctor）
...
## [ERR-20260923-001] 九儿核实课件数字跨日错比——260923 课件的「今日 1.77 万亿」错拿去和 9/22 的 2.14 万亿比，误判「课件错误」并错误撤下已入库数值

**状态**: ✅ 已闭环（Doctor 2026-09-23 语「应该固化完成了」 · 句芒未参与实施·实读核验代记——live ingest SKILL 已含「日期指涉核对」护栏段、与建议修法①②③逐条对应；brain live 镜像为派生只读副本、待 Doctor 终端 rsync 刷新，命令已随本班 git 块贴出；可回退·Doctor 一句话可改）

**触发/硬证据**: 2026-09-23 课件入库班后，Doctor 令「联网核实 260923 课件连板句矛盾」→ 九儿核实 9/22 数据（2.14 万亿放量、6 板）后，把 260923 课件的「今日成交 1.77 万亿、较前一日缩量 17%」与 9/22 对比 → 判「课件数字错误」，撤下 dim3 2026-09-23 行 volume_trillion=1.77（修前 (1.77)→(None)）。Doctor 追问后重新核实 9/23 真实盘面：两市约 1.76 万亿（三市 1.78 万亿）、较 9/22 缩量约 3705 亿≈17%、涨停 51、跌停 14、最高板 4 板——**与课件完全吻合**（2.14×0.83≈1.77 恰为缩 17% 值）。修后回滚：volume_trillion=1.77、consecutive_boards=4。

**根因（三层叠加）**: ① 框架先入：核实搜索上下文全是 9/22 数据，遂把 260923 课件也锚定在 9/22（用「美股 7 点」时区猜测支撑「同日两版」假设），文件名前缀 260923 这一默认证据被复杂假设覆盖；② 证实性偏差：带着「再揪几个课件错误」的倾向取数，「1.77≠2.14」被顺势读成课件错误而非日期错位；③ 奥卡姆剃刀用反：自算的 2.14×0.83≈1.77 精确吻合本是最强反证（乱写不会恰撞缩量 17% 的乘法关系），却被读成「AI 把明日预测写成今日事实」。诱因（非根因）：哥哥 09-22 漏放课件、09-23 补放，两份连续日期文件同班 ingest，内容重叠（新华传媒/PCB 跨日连热）助长「同日」错觉。

**影响面**: dim3/dim2/recap_daily/dim4 的 2026-09-23 行短暂写入「课件数字不符」的错误注记约 1 小时，期间若被消费会读到错误定性；数值列 volume_trillion 一度被撤。当日已全部回滚修复，真库 integrity ok、行数不变校验 8 表 OK。

**建议修法/预防门禁**: ① 核实课件数字前先钉死「课件说的哪一天」（kejian_date + 内容『今日/明日/前一日』指涉）再取对日数据；② 同班 ingest 多份课件时逐份先核对日期指涉再提炼；③ 发现精确吻合先读成简单真相（奥卡姆剃刀），不先构造复杂解释。①②已落本班日志「流程护栏」段；是否将护栏固化进 ingest SKILL 待 Doctor 裁（prompt 改动走 Doctor 终端）。
