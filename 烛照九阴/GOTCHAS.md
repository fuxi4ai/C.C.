...
## [ERR-20260731-001] `intl_index_daily::US10Y`(^TNX) 与 FRED `DGS10` 是**两套时点语义**——平日差 0~1bp 骗过校准，**事件日劈 5bp**
**状态**: ✅ 已定性（📌 成因中「15:30 ET」一项待坐实，见下）
...
**根因（同名异义 · ERR-20260728-005 家族）**: `DGS10` 是财政部**固定期限收益率**，H.15 原文 *based on the closing market bid yields*，业界通行口径为 **15:30 ET 买方报价快照**；`^TNX` 是 CBOE 指数、**16:00 收盘**。平日尾盘无事两者贴合；事件日最后半小时一动即劈开。**📌 待坐实**：「15:30 ET」系按业界通行理解写入，正式引用前应去财政部 *Treasury Yield Curve Methodology* 页核。

**⚠ 时点差不足以完全解释**: 2026-07-30 会话记录的本地 07-29 盘中最高为 **4.651（12:10）**，低于 4.67 —— 4.67 落在本地全天区间之外。两种可能：那份盘中序列在 **14:55 被截断**（未含决议后段，最可能），或某一侧数据有问题。**需重取 07-29 完整分时（含 14:55→16:00）方可定性**，在此之前不作结论。
