...
> 5. **双向闭环关闭**:每个 requirement 逐项 `[✓]`，或被字段齐全的合法总签明确覆盖；另一合法关闭路径为 Doctor 显式取消(原因+时间)。**禁止 CC 自动关闭**
...
> - 「指定独立验收方」**必须由 Doctor 指定**；独立审查员默认只有背书权——2026-08-29 常设授权：事实性/事务性/有最优解的事项（普通 TODO、gotchas 状态行、定时任务事务等）在可回退基础上，由未参与实施的 subagent 独立审核后代签代勾；PRD 内功能性 checkbox 与方向/方法/质量判断仍归 Doctor 或指定独立验收方。
...
- 「指定独立验收方」必须由 Doctor 指定；独立审查员默认只有背书权——2026-08-29 常设授权：事务性/可回退项（普通 TODO、gotchas 状态行、定时任务事务等）由未参与实施的 subagent 独立审核后代签代勾；PRD 功能性 checkbox 与方向/方法/质量判断仍归 Doctor 或指定独立验收方。
