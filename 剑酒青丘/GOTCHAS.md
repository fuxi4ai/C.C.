...
**修复/纪律**：① 勘误后必 `grep 旧值全文 = 0`（含否定语境逐一核对）才可称完成；② 任何「跑完/可执行/已复现」类表述前必须先实跑；③ 大改动配未参与开发的外部审查（本 session 已立为 EXP-20260815-008-P）；④ 载体同步验收**先 canonical→Brain artifact→Gateway index→latest 查事实指纹，再做 cmp/hash**（2026-08-17 VV 终验补）。
