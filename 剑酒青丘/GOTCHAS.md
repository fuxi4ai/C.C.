...
**修复/纪律**：lock 重写为传递依赖锁；声称「干净环境可装」前必须干净环境实装验证；「八文件包、七项 SHA 通过」类表述数清 payload 数。**强化（2026-08-17 VV 四轮）**：干净环境实装、锁文件与 payload/SHA 一致性——锁文件数目与 SHA 清单行数必须与 payload 逐一对应。
...
**修复/纪律**：EAL artifact 变更 = **两处全推（brain 镜像 + Gateway index）**，且声称同步前逐文件实读核验；**versions/*.html 由 update_artifact 自动轮转（更新前 index 的 N−1 回滚副本），禁止人工覆盖、不参与同代判定**（2026-08-17 VV 二轮终验订正）——与「结构改动须 artifact+prompt 双落」铁律互补：双落管机制、实读管宣称。**强化（2026-08-17 VV 四轮）**：改为 canonical→生成器→Gateway active→mirror/snapshot 的**章节级回读规则**——发布后按章节逐级回读实际消费端，不比 hash 了事。
**同族/来源**：RISK-20260817-002（宣称完成未核）。→ 同上
...
**同族/来源**：台账 v2.3 勘误（Pearl 中介边界）· G-X112（口径四问的因果版）· VV 四轮六根因表「因果边界失守」

### [BUG-20260817-005] 正常路径全绿不证明 fail-fast（2026-08-17 VV 四轮立）
**状态**：🔄 已修待独立验收（2026-08-17 · 十二修 · test_failfast.py 10 用例固化·SHA 八项含测试件；**降档注（VV 五轮）**：测试未覆盖乱序/重复键/真实 -O 子进程·测试自身依赖 assert 致 -O 假全绿——五轮整改补修中）
**优先级**：🔴 高
