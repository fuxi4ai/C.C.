...
- [ ] **EAL 收口余尾（Doctor 动作条 · 2026-08-18 /save 更新）**：① Settings 七轮版已贴 ✓ + 新会话注入核验 ✓（08-18 resume 场逐行比对：块二存在·无六轮旧措辞·漂移零）；② ~~live 镜像 21 班同步验证结果知会留档（--dry-run→全量后）~~（✅ 2026-08-26 问答板 3A 裁销账：数据链班已实证闭环（08-25 shadow frozen_events=11 · artifact updatedAt 08-26T04:45Z），不再补知会 · CC 代记留痕）；③ 重建产物单独 commit ✓（已核实·gitcheck.py 2026-08-18：commit「brain: 重建产物再落盘」在 HEAD 链 · push 已同步 · worktree 仅 TODO.md 留痕）。
...
- [ ] **渊图 · 胜宏科技 M9/M10 认证法定披露校验窗口（2026-08-05 挂 · 2026 年 8 月底半年报）**：核实札记已定级——「M9 材料产品验证完成」P2 可坐实，「全球唯一通过英伟达 Rubin 正交背板验证」坐不实（公司 03-18 法定口径「正积极推进」、07-31 董秘回避）。**8 月底半年报是第一个法定披露校验点**：届时核对 M9/M10 认证表述是否升级，据此决定 hedged patch 起草/作废。旁线：SemiAnalysis「Rubin 因正交背板卡点延迟一年量产」（P1 外媒，与「已过验证」叙事正面冲突）若被英伟达官方/供应链否认 → 反向信号降级；若被证实 → 本身即图谱级信号（正交背板卡点 → VR200 子图），可另立核实。**⇒ 2026-08-19 新增对照素材**：帕米尔 08-18 PCB 调研口径「胜宏 Vera Rubin 批量交付 & 高多层份额提升」（调研口径 P2·非法定）已入库——半年报校验时与此调研口径对照，若半年报仍无 Rubin 认证升级表述，则「批量交付」维持调研口径不升格。**⇒ 2026-08-25 新增对照素材**：SemiAnalysis 7 月爆料「Kyber NVL144 正交背板卡点延迟 12 个月+ → 2028」部分证实且范围收窄——延迟仅 Ultra 相，2026 主代 Vera Rubin 5 月底已规模量产（黄仁勋 7/10 澄清 2027 轨道不变·Plan B 降层数/回铜缆）；对胜宏 M9 材料的双重含义：认证需求推迟一年 + Plan B 材料降级风险。详见 `raw/核实/2026-08-25-VeraRubin实测与AI服务盈利核实札记.md`。**⇒ 2026-08-25 校验场预核（CC 实查）**：披露日官方钉死 **2026-08-28**（互动易 07-31 答复 + 08-07 异动公告双源 P0）；期间无可靠新料——08-24 韭研公社聚合帖「M9 获 N 认证·仅胜宏小批量出货」定级 P3 不入证据链（来源未知+作者持仓+主语混乱+借名「天风郭明錤」发 M10 测试消息）。**核验靶点清单（08-28 披露后逐项核）**：①半年报 M9/M10 认证表述 vs 03-18 基线（「M9 推进中」）——升级一档即可坐实帕米尔「批量交付」调研口径，仍无表述则维持调研口径不升格；②图内 `company_Shenghong`（Rubin Compute Tray 一供 75% 调研口径 props）；③图内 `company_VictoryGiant` desc「正交背板独家供应商」强断言与景旺送样节点（`concept_RubinUltraOrthogonalBackplane` 景旺 78/84/108 层送样）图内矛盾；④`concept_PTFETestProgress2026Q3`「7-8 月确定最终方案」dated 断言**已顺延**（8 月口径 Q4 或之后·英伟达未官宣 BOM 锁定仅立验证 spec）——须与半年报 PTFE/M10 表述分开核（M9 碳氢+石英布=量产主力 vs PTFE 无布=M10 级候选）；⑤**双胜宏节点并存**（VictoryGiant 06-19 建 / Shenghong 08-18 建·aliases 互指——同名双 id 族系）手术待裁；⑥VictoryGiant region=港 疑港股代码 02476 误配（A 股 300476 应=中）；**⇒ 2026-08-27 靶点增补（CC 实读新挖·C 档·札记 §6）**：⑦**双节点并存族**（超出靶点⑤双胜宏·同族手术待裁，勿与 08-28 核验混批）——双生益（`company_Shengyikeji`/`company_ShengyiTechnology`）·双菲利华（`company_Feilihua`/`company_PhilipsGlass`）·双日东纺（`company_Nittobo`/`company_NittoboGlassCloth`）·双宏和（`company_Honghe`/`company_HongheElectronicCloth`）·双斗山（`company_Doosan`/`company_DoushanCCL`）·双泰山（`company_TaishanGlass`/`company_TaishanGlassFiber`）·双巨石（`company_ChinaJushi`/`company_JushiGroup`）·三光远（`company_Guangyuan`/`company_HenanGuangyuan`/`company_LinzhouGuangyuan`——光远新材位于林州·疑同实体三节点）·三 Q 布（`material_QGlass`/`material_Q_Cloth`/`material_QClothQuartz`）·双 T 布（`material_TGlass`/`material_T_Cloth`）；⑧边 id 与端点不一致 2 条：`rel_Jingwang_VeraRubinMidplanePCB`、`rel_Jingwang_OrthogonalBackplane`（id 带 Jingwang·source=company_Pengding——景旺↔鹏鼎混标族）；⑨语义污染边 `concept_VeraRubinMidplanePCB -is_a-> product_Midplane`（后者是人形机器人中板·高盛机器人报告源）+ 方向疑点边 `concept_RubinSwitchTrayMaterialShare -used_in-> material_M7M8CCL` + `company_ShengyiTechnology` desc 主语漂移。**⇒ 2026-08-26 /todo 裁定并入本场**：烛照 SA 应急通道 QQQ 口径污染根治（QQQ 值写入覆盖 ^NDX 行 · 是否另立 code）由 Doctor 裁「并入 08-28 核验场一起裁」。
  依据：`Database/行业研究/raw/核实/2026-08-05-胜宏科技Rubin验证核实札记.md` · `logs/2026-08-05-渊图-胜宏科技过验证P2核实.md` · `raw/核实/2026-08-27-VeraRubin PTFE材料与CCL电子布影响核实札记.md`（§6 C 档）
...
- [ ] **龙鱼 · ds 腿引用数字检测机制（2026-08-20 补挂 · 源：NOTE-20260819-001 · 原 08-15 挂条已从 TODO 消失）**：ds/LLM 盲打 evidence 中具体数字（产能/订单/营收额/市占率）须能在 canonical/engine_facts 找到同源，找不到标 P3 降权或不采；机制化=校正/销账审计逐句对 canonical。同族两例：08-15 华工「2万样机」P3 当事实、08-19 海光「中芯产能」错链锚（华为链·海光代工实为三星）。**⇒ 2026-08-26 /todo Doctor 裁「暂缓」**：等校正/销账审计自然推进时顺带机制化（非阻塞项）。
...
- [ ] **渊图 · 存算三孤岛节点补边题材（2026-08-25 由 /todo 漏挂对账补挂 · 源：`logs/2026-08-22-渊图OCS深挖与德科立300台核查修复.md`）**：SamsungHBM_PIM/NeuRRAM/中国存算一体需求红利三孤岛节点补边题材——内容工作，等 Doctor 提起。
