...
- [ ] **PEC · D4 重跑（2026-09-26 开跑 · **已暂停待裁** · 源：`logs/2026-09-26-PEC阶段1清空与门禁收口.md` ＋ 本场）**：**Doctor 已裁开跑**，本场按「分批 · 中华打头」跑了 **CS-05**。**⚠ 结果＝尺未定，D4 暂停（Doctor 2026-09-26 裁）**——`raw/2026-09-26_analysis_CS05判定重跑.md` 经**三轮独立复验（FAIL / PASS_WITH_LIMITS / FAIL）**，根因收敛为一条：**晋 v2.2 的判据集在「入场之后如何计数」一段上不足以保证一致裁决**（B1 表单管入场、规则二管历史计数，两者无映射；`§3.3 L252` 计数门只给方向不给可核判据）。**Doctor 已裁尺＝「用 D1 句式定义反向统一」**，落法已写入该件 §五（句式分流：应然句计入 `i`／实然句归表现层／原因条件句不入候选）。**⇒ 状态升级（2026-09-26 三轮问答后）＝不再只是「待尺」，而是「待两项真前置」**：① **术语-证据映射**（候选名 ↔ 原典用语）——否则六项里**五项结构性假阴性**（候选名是现代分析标签、原典用别的词；`大一统` 命中 0 因其出处《公羊传》「大一统也」未被册子引录）；② **传承文本语料**（原典原件入案级层）——否则假阳性无过滤（`文字-史学` 12 命中里含 `坛经`「**非关文字**」「**不立文字**」＝**语义相反**的引录被算成支持）。**实测缺口（`CS05判定重跑.md` §5.7 已盘点）**：`读懂中国/03_史书传统/` 引录**只 10 行**（而正史是六项核心证据库）· `共享基础/sources/` **空目录**（三次实测）· `衍生传播/` 空 · **十三经除《诗经》《论语》片段外全缺** · **制度文献（会典／诏令／律令／家训／蒙学）全缺**。**⇒ CS-01／02／03 与 25 条表一并冻结**，解冻判据＝两项前置补齐。**⚠ 另查出 PRD 排期一处循环依赖**：`PRD §6.3` 把 **T1（`_映射表.json`）排在阶段 2**（D4 之后），而实测 D4 的机读执行**需要**术语映射 ⇒ 阶段 1 依赖阶段 2 的产物。**同挂**：**D5 件**待 Doctor 落签 · 门禁侧 `丙`/`9(a)`/`9(b)`/`8(c)`/`G-1`~`G-5`/`D3` 待签（登记于 PEC `文明基因历史沿革.md` §五）· `G-31` 是否升格 · `§六 L301` stale 处置（**本轮已实核确属 stale 且已标注**，处置方式待裁）。**✅ 已办**：形成史件 **13 处回改**（D6 三处＋自审 F5／F8 连带＋三轮复验逮出的 G3／同义残留／钟摆型三写法）· **§五 整节 + §十二 已加 stale 标注**（Doctor 裁「标注不动内容」）· CS-05 判定件 v4 落盘（含 §5.5 跑尺失败证据 · §5.6 换尺小样 · §5.7 缺口盘点）· 一次**自我纠正**已留痕（曾误指探针编造「孙诒让」，实核在 `A01` L694）。**⚠ 本场另查出判据集一处体系缺口**（尺已裁但落实待验）· **该件 v1／v2 未留盘、不可复现**（已如实登记）。**同挂**：**D5 件**待 Doctor 落签 · 门禁侧 `丙`/`9(a)`/`9(b)`/`8(c)`/`G-1`~`G-5`/`D3` 待签（登记于 `Projects/PEC/文明基因历史沿革.md` §五）· `G-31` 是否升格（K 指出真正的字面共用发生在项目内部）· `§六 L301` 是否属 stale（**本轮已实核：确属 stale**，涉「底层＝持续的选择者；表层＝被选择者——可由规则一导出」，规则一已降为记录项；已在形成史件回改留痕第 12 处覆盖，**处置方式待裁**）。**✅ 已办**：形成史件 D6 十三处回改全部完成（含自审件 F5／F8 连带与三轮复验逮出的 G3／同义残留／钟摆型三写法），改前 SHA 均经写前复核。
...
- [ ] **巡检 · 第三轮独立复验 + G-X193/audit 三行挂账（2026-09-22 /save 挂）**：二轮独立复验（PASS_WITH_LIMITS）之后我又**连改三轮设计**（作废 `boottime` 降级 → 作废时段窗 → 机器状态归因；另加 trade_cal 与 brain-resume 附报），**全部只有自验**——按纪律只写「已修改、自验结果、待独立验收」。**验收判据**：下一轮独立复验报告 ＋ 下次周班（2026-09-27 20:00 PT）面③ 实证。**同挂**：`通用教训` G-X193（🔄）· `_repair_audit.md` 三行 🔄（03:02 / 04:38 / 05:05）——✅ 按分轨：事务性／事实性的由未参与实施的 subagent 代签；方向性的问 Doctor 或指定独立验收方。
...
- [ ] **通用教训 · G-X83 状态退签（2026-09-26 由 /todo 漏挂对账补挂 · 源：`logs/2026-09-23-五仓提交与探针纠错.md`）**：09-22 的 ✅ 是否应退 🔄——归 Doctor 或指定验收方（按分轨：事务性／事实性的由未参与实施的 subagent 代签；方向性的问 Doctor）。
