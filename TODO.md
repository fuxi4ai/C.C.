...
- [ ] **渊图 · 08-08 三轮叠加清洗与结构修复专场（2026-08-09 挂 · Doctor 批四项全立 · 源：渊图_GOTCHAS NOTE-20260808-001）**：同一 6 篇当晚被入库三轮（21:28 promote 成功 → 23:12 重跑 promote → 23:38 被 kg_promote 拦死）留下的账——① **价格层清洗**：冗余 12 条删冗（油运 4 事实×3 轮=12→留 as_of 全对齐文件日期的轮 C 删 8；药明 2 事实×2 轮→留信息全者删 2；食蟹猴同价两录→留分档细粒度三条删 2）＋老账 5 组「同 id 不同事实」改 id 双留（DRAM 三星 Q1实际/Q2预期、MLCC基粉已涨/预期、光模块PCB 现价/前价、T布已涨/可承受、空心杯 德昌/鸣志）；清洗清单 CC 已出待 Doctor 审，审后出脚本终端跑（含备份 + latest.json/price_index.json 同步重生成 + 行数守恒校验），预期 774→762。② **图谱层硬重复手术**：23:12 轮带 ~7 对同名同义双 id 过门进 canonical（谷歌Willow芯片/抗量子密码/量子密钥分发/量子随机数生成/量子计算人才稀缺/药明TIDES在手订单2026Mar/激光通信主干链路）＋~5 对近义边缘（离子阱工程化、太空散热/储能/抗辐射等）——kg_promote 结构校验查不出语义重复；手术=合并/删节点+边重定向，规格另拟（大活）。③ **机制修复**：kg_ingest 价格钩子加幂等查重（同事实键不落第二条）＋ CLAUDE.md 入库流程加防重跑保险丝（batch 前查拟处理文件 kg_processed 全 true 即停）。④ **存量同名双 id 26 组**（深南电路三 id、兴森/生益/伟测/洁美/中巨芯/中船特气双 id、鼎泰高科 DingTaiTech/DingtaiHighTech 双黄蛋——06-28 命名铁律正主也没躲过）随 ② 并场清。
