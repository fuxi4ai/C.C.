...
- [ ] **brain · 8 个项目目录缺 `architecture/系统概览.md` → `brain-save` Step 4 对它们一直空转（2026-07-31 挂 · 由 G-X111 补扫查出 · 同日 ⚠ 名单订正 · ② 已落盘待您勾 · ⇒ 2026-08-13 问答板裁：先补 3 份活跃最高〔烛照九阴/风险日报/剑酒青丘〕）**：`brain-save` Step 4 规定「同步项目状态 → 更新 `{项目}/architecture/系统概览.md` 的最后活跃字段」，缺文件即静默跳过、无人察觉；`dashboard-snapshot.py` 的项目卡也少一个信息源。
...
- [ ] **跨项目 · `.bak_audit20260728` 另 10 个散在 5 处（2026-07-30 挂 · ⇒ ✅ 已清（Doctor 终端） · 2026-07-31 补挂 TODO · 2026-08-03 /todo 重取计数）**：风险日报 ×1 / 白泽大宗 ×3 / brain ×3 / Market-Data ×1 / **烛照九阴 archived ×2**（原条写「另 8 个散在 4 个项目」系 07-30 快照，archived 2 件当时在清单外）。同批产物应一起清，但跨 5 处，**不该在单项目自查名义下顺手做**，需单开一场。
...
- [ ] **金融 · macro_prediction.db 择一为源（2026-08-08 挂 · ⇒ ✅ 已执行：v3 为源 + v1 已改名 deprecated（Doctor 终端））**：v3（macro_predictions_v3）是白泽观星引擎 engine_cn.py 写的新版口径（L662 建表），v1 是旧表；消费方=白泽观星。**待裁**：v3 为源 + v1 标 deprecated。

- [ ] **dev/19 班迁移收尾杂项包（2026-08-09 由 /todo 漏挂对账补挂 · 源：`logs/2026-08-02-dev模式打通四件套.md` ＋ `logs/2026-08-02-E验收与19班迁Kimi壳.md` ＋ `logs/2026-08-03-数据根迁Gateway-workspace.md` · 多为 Doctor 终端小动作）**：① ~~沙箱垃圾清理~~ ✅ 已清（Doctor 终端）；② 字节级终封 diff 复跑（touzhijunjun 修正后应 19 OK）；③ longyu 步骤 2/3 修正状态核实；④ 龙鱼看板幽灵归档（`mv ~/Gateway-workspace/Artifacts/龙鱼五力个股库…`）；⑤ V.V. 盲审回收 → ⚠️ 08-13 完整性阻断（VV 见封存归因），Doctor 出新盲测包重审；⑥ workspace 7-05 旧副本处置（F 方向未批，先留定性）；⑦ `Brain`/`brain` 大小写归一（Linux 化前必做，本轮记档不动）；⑧ Settings 镜像 L20 过期提醒撤除（1 行改动，待批）。

- [ ] **基建 · 看门狗挂载治理 · 重开（2026-08-09 目标模式裁定重开 · ⇒ 2026-08-13 问答板裁：开专场查授权固化 · 前条已闭合：归档「看门狗班沙箱挂载缺失治理」08-03 挂 · 08-08 Doctor 勾定迁档）**：前条闭合方案（前置确认段）未根治授权固化——2026-08-09 白泽周日班 01:09 又因三目录未挂载阻塞（`logs/2026-08-09-白泽周报阻塞-挂载缺失.md`，当日晚 Doctor 补挂后补跑成功）。待做：查 scheduled task 目录授权固化机制出方案 + 同族扫其他班是否同缺（G-X111）；与 L37 巡检适配并观。
  **⇒ 2026-08-11 目标模式现核：定为大活 · 转专场（退出「清到0」范围，另批专场）**

- [ ] **白泽大宗 · P2 缓项包（2026-08-10 挂 · ⇒ 已做 2026-08-13：NULL-source 20 行回填「人工配置」+ data_source 归一 + config_index 补 benefit_relations）**：剩 ingest_meta rows_written 语义 + macro v1/v3 注记（随 macro 择源）+ data_hash 隐患（冻结表，若手动跑旧 incremental_updater 会误判全变）。
  **⇒ 2026-08-11 目标模式现核：定为大活 · 转专场（退出「清到0」范围，另批专场）**
...
- [ ] **龙鱼 · 天孚/仕佳入标的库光通信组评估（2026-08-12 挂 · ⇒ 已入组 2026-08-13：细分环节.json 映射已做 + records 生成 + build_index 重建 118 标的含天孚/仕佳待打分；剩看板重建推送（Mac 终端跑 build_dark_board）+ dualscorer 六维全分（周六班））**

- [ ] **渊图 · 兴森「广州兴森 2025 收入 2689.99 万/净亏 5.33 亿」两数待核（2026-08-12 由 /todo 漏挂对账补挂 · 源：`logs/2026-08-12-ABF清洗与canonical入库.md`）**：VV 标 P0、CC 标 P0-待验。**⇒ 2026-08-13 tushare 核证**：合并锚营收 71.95 亿/净亏 -1.04 亿（ann 2026-04-25），fina_mainbz 无子公司拆分；「收入 2689.99 万」量级存疑（IC 载板子公司收入不可能仅 2689 万）。⇒ 2026-08-13 tushare 产品拆分核证：IC 封装基板 16.70 亿/-2.68 亿毛利 →「收入 2689.99 万」确证有误、「净亏 5.33 亿」量级合理；落 canonical 注记。

- [ ] **金融 · 9 月加息概率实时数值源未核（2026-08-12 挂 · ⇒ 已查明 2026-08-13：Kalshi API 可通但 26SEP 利率市场订单簿全空（无交易=无定价）；预测市场对 9 月会议无有效概率。⇒ ✅ 已核（2026-08-13 CME FedWatch 官方·Doctor 浏览器读数）：9/16 会次 hold 65.6% / 加 25bp 34.4% / 降息 0，落 fomc_market_exp.json；检索快照 p≈50% 偏高，官方口径 ~34.4%）**
- [ ] **渊图 · 云锗「InP 业务贡献多少业绩」未核财报（2026-08-13 挂 · ⇒ ✅ tushare 核证：化合物半导体材料分部 2025 收入 1.38 亿（占 12.9%）·分部利润 3217 万；「5.7-8.55 亿」系远期长单金额非当前收入）**
