...
- [ ] **★ 渊图 · `concept_XinsenBTSubstrateCustomerShare` 畸形节点还原 —— 11 个节点静默丢失 26 天（2026-08-01 挖出 · Doctor 定甲案「本批不动、另开一场」· ⇒ ✅ 节点还原已完成 2026-08-13：11 节点入图 + 16 边 + 6 update + 帝尔归正并入 company_DRLaser，剩 QA 第9/10项机制修复未做）**
  ...
  **⇒ 2026-08-12 追加（ABF 清洗实证 · Doctor 确认挂载）**：QA 除第 9 项外再加**第 10 项——`data_sources.file` 落盘存在性断言**：引用文件名做 raw 全库可解析性检查、不存在即报。实证依据：「ABF膜学习材料」18 节点 +「兴森科技-学习材料」21 节点两批幽灵零告警躺数月；分级处置范式已入 EXP-20260812-002-P。
...
- [ ] **龙鱼 · CPO 六只跑分数据包落库 + 推 08-15 周六 dualscorer 补 LLM 全分（2026-08-12 /save 经 Doctor 勾选挂载 · 源：`logs/2026-08-12-CPO阶段汇总与源杰深挖.md`）**：8/12 沙箱实跑引擎数据包 6 只（天孚 4.5/源杰 3.5/仕佳 4.5/光库 2.5/罗博特科 1.0/炬光 3.5 · 财务健康分）暂存会话 outputs，未落 `龙鱼五力/reports/`。待做：① 数据包落库（**罗博特科已按 Doctor 裁定剔除名单，不落不推**）；② 推周六 08-15 dualscorer 班补 LLM 全分——**注意光库 07-14 已有 CC 六维分 62.5 谨慎**（系统概览实证），补分名单=天孚/源杰/仕佳/炬光 4 只（光库重打与否随班裁）；③ 源杰本就在双 scorer 待校正队列（07-12：Δ−19 疑 deepseek 过罚），本次补分一并了。

- [ ] **龙鱼 · 天孚/仕佳入标的库光通信组评估（2026-08-12 /save 经 Doctor 勾选挂载 · 源：同上）**：8/12 CPO 纯度鉴定出纯正 7 只，其中天孚通信 300394、仕佳光子 688313 为库外标的（光通信组现 9 只无天孚，缺口明显）。待做：评估入库（含 `_index/细分环节.json` 映射 + build_index 重建 + 若走 dualscorer 一并打分）；分组改动遵三处同步纪律（json._meta↔模板↔重建推送）。

- [ ] **渊图 · 兴森「广州兴森 2025 收入 2689.99 万/净亏 5.33 亿」两数待核（2026-08-12 由 /todo 漏挂对账补挂 · 源：`logs/2026-08-12-ABF清洗与canonical入库.md`）**：VV 标 P0、CC 标 P0-待验。**⇒ 2026-08-13 tushare 核证**：合并锚营收 71.95 亿/净亏 -1.04 亿（ann 2026-04-25），fina_mainbz 无子公司拆分；「收入 2689.99 万」量级存疑（IC 载板子公司收入不可能仅 2689 万）。两数仍 P0-待验，待年报原文终核。

- [ ] **渊图 · 幽灵 file 专项审计（2026-08-12 补挂 · 2026-08-13 已执行首轮）**：实测 58 真幽灵（571 引用去重 = 165 web + 25 跨命名空间 + 381 文件引用）。已处置：校正 7 个 file 值（99 处 data_sources）＋标 unresolvable 49 个（521 处），canonical 备份 `.bak_20260813_幽灵处置`。剩：图手术档（畸形节点 11 还原 + 双 id 28 组）未做。

- [ ] **金融 · 9 月加息概率实时数值源未核（2026-08-12 由 /todo 漏挂对账补挂 · 源：`logs/2026-08-12-7月CPI实核与EAL传导力分析.md`）**：p≈50% 现为检索快照，Kalshi/Polymarket 页面 JS 渲染抓不动，待找替代源。
