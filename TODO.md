...
- [ ] **DVA · fuxi 单视频入口（2026-07-25 挂 · 收窄 2026-08-14）**：VV 08-13 已验证通过（Qwen3 转写 4190 字·未 seed 业务库），CC 已出 single_one.ps1 并 scp 至 fuxi。**剩**：single_one.ps1 fuxi 首跑验收（VV）。

- [ ] **DVA · 孤儿字幕②（2026-07-24 挂 · 收窄 2026-08-14）**：21 孤儿目录/150 篇已裁并移交 VV（10 seed〔浪浪65/隐山19/梁州9/7×1〕+ 11 归档 + B0 ghost 删除）。**剩**：VV 执行 + 五读数回报（authors 12→22 · [无法定位作者]=0 · 孤儿目录归零）后闭合。种子作者喂渊图属另场裁定。

- [ ] **金融 · BT-19 观星概率序列 point-in-time 重建（2026-08-14 问答板 Q5B 裁「优先开」· 自「基建与另场专项三件」② 拆出）**：立 PRD（目标+可验证交付标准+验收方式）→ CN 腿首跑 → Fed 腿日更入调度。**优先开专场立 PRD**。原条 ①③ 已收口/随 Q6 终端包。
...
- [ ] **DVA · B 系列收口（2026-08-08 挂 · 收窄 2026-08-14）**：B1-B6 已闭环，B0/B7/PH 已裁（决策记录 2026-08-13 在盘）。**剩**：① B4 16 组改名冲突逐组比对正文（人工）；② VV 执行面（孤儿② seed/归档 + B0 ghost 删除 + single_one 首跑）。
...
- [ ] **dev/19 班迁移收尾（2026-08-09 挂 · 收窄 2026-08-14）**：① 已清。**剩**：⑤ V.V. 盲审——08-13 完整性阻断，**等 Doctor 出新盲测包**；② 字节级终封 diff 复跑 · ③ longyu 步骤 2/3 核实 · ④ 龙鱼看板幽灵归档 · ⑥ workspace 7-05 旧副本处置 · ⑦ Brain/brain 大小写归一（记档不动）· ⑧ Settings 镜像 L20 撤除——小尾随 Q6 终端包。
...
- [ ] **龙鱼 · 天孚/仕佳收尾（2026-08-12 挂 · 收窄 2026-08-14）**：入组已做（细分环节.json 在盘含两标的 + records + build_index 118 标的）。**剩**：① 看板重建推送（您终端跑 `python3 ~/Documents/Database/龙鱼-标的分析库/_index/build_dark_board.py`）；② 周六班六维全分（并入上条）。
...
- [ ] **风险日报 · 成交额定源 2020+ 切 volume_trillion（2026-08-11 挂 · 2026-08-14 问答板 Q5B 暂缓移入）**：08-13 问答板已裁方向（2020+ 窗口分位/回测切 volume_trillion）——待挂载 Market-Data + 方法论复审后执行；触发=挂载就绪。
