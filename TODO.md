...
- [ ] **渊图 2 件（2026-08-08 由 /todo 漏挂对账补挂 · 源：`logs/2026-08-05-渊图3篇入库_CCL年份偏移修补.md` ＋ `logs/2026-08-06-渊图4篇入库.md`）**：① batch 收尾加「解析日期≠文件年份」显式汇总行（07-28 已建议未实装，已三次复发）② 太空算力篇（07-30）处置——2026-08-08 现核 `raw/_hold_跨域待定/` 已积 6 篇（量子科技 07-09／药明康德 07-13／油运 07-16／CXO 07-17／AI 治理 07-17／太空算力 07-30），hold 清算待 Doctor 裁。**⇒ 2026-08-08 Doctor 裁定 6 篇全入库**：已移回 `Obsidian Industrial/`（_hold 清零）；batch 交 Doctor 终端（沙箱无 KG_API_KEY），promote + wiki 后此项可勾。
