...
- [ ] **EAL v3 吸收（shadow.7 · 2026-08-19 VV 交付 · 生产候选 Mac CPython 3.13.3 全套 35/35+19/19+7/7 双模式全绿已验 · 沙箱 3.10 34/35 根因已查实非包缺陷）**：① 吸收执行待 Doctor 确认映射清单——backtest/eal_v3/ 全新落（v2/repro_v23 零触碰）+ Brain canonical（PRD→checkpoints/2026-08-19_EALv3事件冲击与状态跃迁_PRD.md · METHOD_CONTRACT→frameworks/EAL-v3事件冲击与状态跃迁模型.md）→ 迁移后实跑 relocated tests；② 资产清单吸收阶段裁（blocking R6/R8/R9/R14/N1/N5 · Doctor）；③ 分钟级不纳入已裁（Doctor 2026-08-19）；④ §3 数据迁移单开专场（涉生产 DB 维护窗口 · candidate schema/needs-coding intake/frozen registry）；⑤ 27 条 legacy 编码 + PRD 20 R/N 验收随 VV 后续轮。依据：`4AI/Shake hands/to CC/` shadow.7 交接回执 + CC_APPLY.md
