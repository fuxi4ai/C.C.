...
- [ ] **brain · 8 个项目目录缺 `architecture/系统概览.md` → `brain-save` Step 4 对它们一直空转（2026-07-31 挂 · 由 G-X111 补扫查出 · 同日 ⚠ 名单订正 · ② 已落盘待您勾）**：`brain-save` Step 4 规定「同步项目状态 → 更新 `{项目}/architecture/系统概览.md` 的最后活跃字段」，缺文件即静默跳过、无人察觉；`dashboard-snapshot.py` 的项目卡也少一个信息源。
  ...
  **② 已完成（CC 不代勾，请您阅后自勾）**：`brain-save` 已升 **v3.2 / v3.2.1** 并落您账号——Step 4 改为「先 `ls` 探路径 → 不存在则在 Step 6 回报明写 `⚠ Step 4 空转:{项目}/…系统概览.md 不存在`，**不建、不猜、不跳过**」；**刻意不自动补建 stub**（空壳会把「没有全貌文档」从显性缺失变成隐性谎报，比空转更糟）；表下另加硬规「引用该缺口表前先自己重取差集，别照抄表」。原 Step 4 三行一字未改，纯追加。
...
- [ ] **金融 · 时效性记账与核数三件（2026-07-30 从当日三场日志补挂 · 过期即作废或成本剧增）**
  ...
  　**⛔ 连带：P-09 整条撤回（CC 第三次翻面）**。它的地基「10Y 07-29＝4.62、+2bp」**是据本地 ^TNX 外推的、不是读到的**，权威值 4.67 差 5bp ⇒「尾盘长端跳」未被证伪。成因已立 **P-11**（`DGS10` 15:30ET 买方报价 vs `^TNX` 16:00 收盘＝同名异义，平日 7 天差 0~1bp、决议日劈 5bp）＋ 烛照九阴 **GOTCHAS ERR-20260731-001**。
