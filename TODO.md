...
- [ ] **EAL · VV 十三轮修复（2026-08-18 已改 · 已修改自验待独立验收）**：八轮 `894ab9d` ✓、十轮 `0367ec2` ✓、十一轮 `ce63f3a` ✓、十二轮 `9857bd0` ✓（均 Doctor 推送 · 旧命令作废勿重跑 · 当前 HEAD 含值守提交 `cececf0` 路径零交集）；**十三轮 BLOCK（四组合同矛盾）修复已实施**——① 状态唯一真源落净（模板 218/224/235/237+§五+brain-prd 闭环 全部改「frontmatter status 修改 + §四 追加历史行」）；② 关闭合同统一（模板 158/173/237+G-X4 规则 5 改「逐项 ✓ 或被字段齐全总签明确覆盖」）；③ canonical description 去 6 反引号 → 三层重发 SHA `108b43e5…` → save_skill v1.5.3 重发（runtime parsed metadata 与 canonical 可逐字一致）；④ 状态文档同步（SHA/commit/README v1.5.3/视频方法论指针/Settings 镜像补 PRD 立卷与验收边界条）。**待**：① VV 十四轮复验；② 十三轮 commit（命令已贴 · 另起提交 · 排除值守班文件）；③ **全新会话 `/prd` 与「写交付标准」两次路由触发实测**（Doctor 终端 · 十三轮已确认 runtime 可见但未实跑）；④ Claude-3p 从新 .skill 包重装 + plugin cache 回读 SHA `108b43e5…`；⑤ 存量 PRD 处置表待 Doctor 批（下条）；⑥ Settings 镜像已补 PRD 条、待十四轮确认后重贴；⑦ 其他 brain skills 漂移收敛专场；⑧ `⏳` 活状态迁移专场（12 项目）；⑨ dry-run 测试 6 项增强并入 gate 判据集 PRD。
  依据：VV 十三轮终验回执 · `剑酒青丘/GOTCHAS.md` RISK-002 追记六
