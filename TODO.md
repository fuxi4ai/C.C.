...
- [ ] **brain · 15 个注册项目里 7 个缺 `architecture/系统概览.md` → `brain-save` Step 4 对它们一直空转（2026-07-31 挂 · 由 G-X111 补扫查出）**：`brain-save` Step 4 规定「同步项目状态 → 更新 `{项目}/architecture/系统概览.md` 的最后活跃字段」，但实测**缺 7 个**：**烛照九阴 · 风险日报 · 白泽大宗 · 剑酒青丘 · MiroFish · 星空 · 称象**（有 8 个：DVA / GlobalPercent / O MY HTML / PEC / 司南 / 海螺姑娘 / 渊图 / 龙鱼五力）。**后果**：每次 `/save` 到这 7 个项目，Step 4 静默跳过、无人察觉；`dashboard-snapshot.py` 的项目卡也少一个信息源。
  **发现经过（值得记）**：本场 07-31 报告「风险日报缺系统概览」时**只报了那一个、没扫同族**；随后 `/save` 走到 Step 4 才发现烛照九阴也缺，补扫又揪出另外 5 个。**而 G-X111「立规矩时要扫同族实例」正是本场半小时前刚立的条——第一次就没用上**，这条挂账本身即是它的活体注脚。
  **待您定**：① 补写 7 份系统概览（需逐项目梳理全貌，工作量不小、且内容得您把关）；② 或改 `brain-save` Step 4 为「文件不存在则**明确报告**而非静默跳过」（治标但当场可做，且能防止再次无声空转）；③ 或两者都做（推荐先②后①，②可立即止血）。
...
- [ ] **金融 · 时效性记账与核数三件（2026-07-30 从当日三场日志补挂 · 过期即作废或成本剧增）**
  ...
  ④ ★**新增·补核 07-29 与 07-30 收盘 2Y（裁决 P-09 的唯一硬证据）**：基准 07-28＝**4.26**（H.15），07-29/30 尚未发布；FRED DGS2 series 页停在 07-23 旧缓存，须待下一份 H.15。**判读表**：07-29 明显下行（≲4.20）⇒「2Y 全天持平」证伪，④ 证据腿改记「短端跌×长端平＝陡峭化」（**机制不变、签名重写、权重未必下修**）；07-29 持平而 07-30 大跌 ⇒ 回落属次日鸽派修复，并入 P-05 退款腿；07-29 明显上行 ⇒ ④⑤ 整体重做。**在此值到手前 ④⑤ 权重/签名/案文一律冻结。**
