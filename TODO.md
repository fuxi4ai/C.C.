...
- [ ] **EAL · VV 十四轮窄修（2026-08-18 已改 · 十四轮 commit `bc1e86f` 已推送 ✓〔已核实：gitcheck.py 2026-08-18 · HEAD=bc1e86f5c2 · worktree 0 · push 已同步〕）**：八轮 `894ab9d` ✓、十轮 `0367ec2` ✓、十一轮 `ce63f3a` ✓、十二轮 `9857bd0` ✓、十三轮 `bc6bf3c` ✓（均 Doctor 推送 · 旧命令作废勿重跑）；关闭合同两句已统一（逐项 [✓] 或被字段齐全合法总签明确覆盖；另一路径 Doctor 显式取消）· 三层 SHA `f0437ea5…` · runtime cache 已 v1.5.4 单 frontmatter metadata 逐字一致（VV 十五轮读核 · 无需重装）。**待**：① **全新会话两次路由触发实测**（Doctor 分别新开两个 Claude-3p 会话测试 `/prd [测试简称]` 与「写交付标准」· 记录 session createdAt + 实际 Skill→brain-prd 调用 + v1.5.4 新关闭合同命中证据——不得代签完成）；② 完成后 Settings 重贴 + 最终运行时签字（VV 十五轮后）；③ VV 十六轮复验；④ 存量 PRD 处置表待 Doctor 批（下条）；⑤ 其他 brain skills 漂移收敛专场；⑥ `⏳` 活状态迁移专场（12 项目）；⑦ dry-run 测试 6 项增强并入 gate 判据集 PRD。
  依据：VV 十五轮终验回执 · `剑酒青丘/GOTCHAS.md` RISK-002 追记七
