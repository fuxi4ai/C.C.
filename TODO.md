...
- [ ] **EAL · VV 十一轮修复（2026-08-18 已改 · 已修改自验待独立验收）**：八轮 commit `894ab9d` ✓、十轮 commit `0367ec2` ✓（均 Doctor 完成 · gitcheck 实核 · 旧命令作废勿重跑）；十一轮 BLOCK（仓内发布通过、运行时发布失败）修复已实施——模板双轨状态治理（§2.5 无 checkbox 表格 task_id|task_status|evidence · §四 current_status+变更历史 · frontmatter status 唯一真源 · blocked 仅全部剩余需求被阻断 · 关闭条件=逐项验收或合法总签覆盖 · roles+acceptance_authority 指定三字段）；通用教训 :108 关键模板示例改功能主体/:162 补指定独立验收方；G-X136/偏好/镜像「PRD 判断性 gate」→「PRD 内任何 checkbox」；设计提案**全文 superseded**（status 改、banner 落、模板停止引为现行规范）；save_skill **重发 v1.5.1**（content 去 frontmatter·metadata 严格取 canonical·已避免双 frontmatter）；已装 skill 清单 brain-prd 行更新。**待**：① VV 十二轮复验；② 十一轮 commit（命令已贴 · 另起提交）；③ **Claude-3p plugin cache 回读**（单 frontmatter 确认 + 外层 metadata 与 canonical 核对）+ 全新会话实测 `/prd` 与「写交付标准」路由（Doctor 终端）；④ 存量 PRD 处置表待 Doctor 批（下条）；⑤ Settings 等十二轮确认后再重贴（镜像「PRD 内任何 checkbox」口径已同步）；⑥ 其他 brain skills 漂移收敛专场；⑦ `⏳` 活状态迁移专场（12 项目）；⑧ dry-run 测试 6 项增强并入 gate 判据集 PRD。
  依据：VV 十一轮终验回执 · `剑酒青丘/GOTCHAS.md` RISK-002 追记四 · G-X151 追记
