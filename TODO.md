...
- [ ] **DVA · GOTCHAS 双源分叉待裁（2026-09-17 /save 挂）**：brain 索引与 08-06 纪律称权威＝`Projects/DVA/GOTCHAS.md`（最新 ERR-20260911-001 · 09-16 改），但 Codex 交接与本场三条 ERR-20260917-004/005/006 均写 `Codex/Project Mirror/DVA/GOTCHAS.md`（09-1x 已 65 条，Codex 日常在写）——纪律与现实相反，归 Doctor 裁：权威改镜像仓 / 回流合并进 Mac 库 / 双轨各管各。

- [ ] **DVA · Codex 仓 commit+push（2026-09-17 挂 · 命令块已贴 Doctor 终端）**：25 M + 3 新文件（含 Codex 当日 12 处），排除 graphify-out/ops/pycache；随后处置四个 Fuxi staging 目录、`/private/tmp/dva-*-release/` 与 helper 脚本、Fuxi `%TEMP%\dva-writer-role-test-*`。Codex 侧余项见交接件 §8（旧正则两处 · 157→168 期望 · 去 `.cmd` launcher 验证）。
...
- [ ] **9/15-18 双央行周 = 下一个 dated 高危窗（2026-08-03 由 /todo 漏挂对账补挂 · 源：`logs/2026-08-03-日元carry监控注册AI警报.md`）**：BOJ 9/17-18 与 FOMC 9/15-16 重叠周，carry 观察框架日历时点层（BOJ 官网已核全表：8/5 纪要 · 8/10 意见摘要 · 10/29-30 · 12/17-18）。距 2026-08-03 还剩 43 天。（**09-15 已开窗首读**：Fed 加息 92.4-93.6 CME/期货 87-88/Kalshi 83 · BOJ 25bp 完全定价 90-98% · carry 三选二扳机未触发〔USDJPY 155 单日 <0.1% · VIX 17.6 · 30Y +3.9bp〕· 手更不补已裁〔+3.6pp 未到大动阈·决议 09-16 outcome 机制接管〕· 不挂会后快照班已裁。决议：FOMC 09-16 14:00 ET · BOJ 09-18）
...
- [ ] **龙鱼 · 09-05 周六周更班 v3 锚点继承首验（2026-09-01 /todo 漏挂对账补挂 · 源：`logs/2026-08-31-龙鱼ds供需四补v3与检测机制.md` · dated 09-05）**：v3 四补 23 只重跑后首轮周更——验锚点继承（攒满 2 个 v3 点后趋势自动恢复可信）。
