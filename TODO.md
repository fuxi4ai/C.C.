...
- [ ] **渊图按需更新组核实批 · P1 完成收尾（2026-09-13 挂 → 09-13 续跑完成 wave3+wave4 · 源：`raw/核实/2026-09-13-渊图按需更新组核实札记.md`）**：✅ ①wave3 重跑完成（24 篇/151 断言/17 订正落盘·上一轮 3 遗留：猛禽+OpenRouter 落盘、V4.1 时间线裁决坐实）；✅ ②wave4 完成（44 篇/36 订正落盘/2 防诈骗帖跳过）→ **新 116 篇全量核实完成，订正累计 88 处（全经独立 QC+sha256 ledger）**。剩：③ P2 阶段复用 210 篇与 08-16 普查/09-09 十六条批去重口径（另批）；④ ~~入库候选清单三桶~~（✅ 2026-09-13 Doctor 裁「now，批准」→ 已出：`raw/核实/2026-09-13-渊图按需更新组入库候选三桶清单.md`）；⑤ wave2 3 条 unchecked_quota 资产遗失并入 P2 重扫兜底（札记「资产遗失记录」）；⑥ 未核断言补跑（wave3 54+wave4 若干，需 1-2 会话配额·可选）。执行资产：本会话 `outputs/verify-20260913/`（assignments）+ ledgers 在组 `_evidence/2026-09-13/`（wave1-4）。
