...
- [x] **EAL v3 吸收（shadow.7 · 2026-08-19 VV 交付 · 生产候选 Mac CPython 3.13.3 全套 35/35+19/19+7/7 双模式全绿已验 · 沙箱 3.10 34/35 根因已查实非包缺陷）**：① 吸收执行——backtest/eal_v3/ 平铺落位（import discovery 适配+relocation test）+ Brain canonical（PRD/方法合同）✓；② 资产清单已裁（沿用包内冻结清单：SPY+VIX/CL/DGS2/DGS10/SMH · baseline 自资产 trailing mean）✓；③ 分钟级不纳入已裁 ✓；④ §3 数据迁移全链路完成 ✓（sealed `ac00862e…`→candidate→schema 双跑→intake 27/27→编码→冻结 calendar/config→DAILY_SHADOW 首跑 exit=0→load 候选库 76 行→回读 16 表全对）；⑤ 27 条 legacy 编码 pass 1：3 frozen（FOMC 3/6/7 月）+ 24 exclusion 留 intake（时钟/共识点值/未来事件等 · coding_notes 在盘）。**✅ 升级实施收口（2026-08-19 Doctor「继续，直至升级结束」授权下完成）· 编码 pass2 后 frozen 8 条/排除 19 条、load candidate2 4 簇 48 结果 · 待独立验收：PRD 20 R/N 全部 [ ]（VV/Doctor 落 ✓）· additive schema 生产采用（读者切换）待 Doctor 后续裁定 · 07-29 大簇完整结果待收盘数据入 sealed 后随值守班重跑（正常运行非 TODO）**。依据：shadow.7 交接回执 + CC_APPLY.md + §3 实跑证据（PRD §六 变更历史三行）
...
- [x] **Shakehands 清理 PRD 待 Doctor 总 ✓（2026-08-19 会话二执行 · awaiting_acceptance · 判断性验收归 Doctor）**：`logs/checkpoints/2026-08-19_Shakehands清理_PRD.md`——删除 33 项（71→38 · 实核 find 全树 38 与声称一致 · archived 16 封未动）· 保留 20 项零缺失 · 独立审查员背书通过（并揪出 archived 18→16 笔误已修）· 5 条客观轨待 Doctor 签一个总 ✓。**✅ 2026-08-19 Doctor 会话签署总 ✓（原话「签」）· CC 代记留痕 · PRD status=delivered · result=通过 · 连带裁定：真删不用知会 VV（Doctor 定）**。
