...
- [ ] ★ **`us-close-backfill` 首跑核验（2026-07-30 挂于日志 · 2026-07-31 补挂 TODO · 时效：今日）**：该班 07-30 建、`lastRunAt` 字段至今不存在＝**从未跑过**；`nextRunAt` = 2026-07-31 20:34 UTC（**13:34 美西 / 北京 08-01 04:34**）⇒ **首跑就是今天**。跑后核：简报格式与验收项够不够用、`--from=max+1` 自愈是否生效、四条铁律（绝不编数 / 不改脚本 / fail-visible / 不跑 git）有无被触碰。**注**：该班 13:30 美西＝16:30 ET，在 `^TNX` 15:00 ET 收市之后，取到的是真收盘，这条链路本身无需改（2026-07-31 实测）。
