...
- [ ] **PEC · D4 是否开重跑专场（2026-09-26 /save 挂 · 源：`logs/2026-09-26-PEC阶段1清空与门禁收口.md`）**：**阶段 1（判定层解冻）已全部闭环**——D1 定稿「两截」· B1 入场表单采纳 · B2 余项（跨文明层／集合体）移出基因序列 · B3 确认关闭 · D2 补拍 · CS-08 定性「判据换代」· D5 四轴元清对照 v2.2 ⇒ **D4（25 条 + 九文明逐条判定重跑）前置已清空，是否现在开专场归 Doctor 裁**。**⚠ 判定面比原估宽**：含 `大一统` 在内的 **CS-05 六项须整组重判**（判据链是**已降级的规则一**）。**开 D4 前建议顺手清**：`形成史件 D6` 的 3 处未回改（§八「最大空白」／规则零正文与归类表／`[~]` 计数）。**另待落签**：**D5 件**（v2.2 · 三轮复验）· 门禁侧 `丙`/`9(a)`/`9(b)`/`8(c)`/`G-1`~`G-5`/`D3`（登记于 `Projects/PEC/文明基因历史沿革.md` §五）。**同场另挂**：`G-31` 是否升格（K 指出真正的字面共用发生在项目内部）· `§六 L301` 是否属 stale（未核，若属则「判据换代」反证权重下调）。
