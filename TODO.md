...
- [ ] **金融 · A股首样本记账（2026-07-30 挂 · Doctor 2026-08-03 定补记另开专场 · ②③④ 已收口 2026-08-13 目标模式收窄 · ⇒ 2026-08-13 问答板裁：补记，带混杂标注）**
  ① A股首样本：「暂停转鹰 × 事前负 × hold」＋事后首日弱格 obs，必须带混杂标注——美军空袭伊朗发生于北京 07-30 08:00–10:00，A股是带弹交易，属混杂事件日（BT-18 预注册记账纪律）。
...
- [ ] **`us-close-backfill` 与 `zhuzhao` 双写者职责未理清（2026-08-01 挂 · ⇒ 2026-08-13 问答板裁：单开一场定权属）**
...
- [ ] **龙鱼 · 天孚/仕佳入标的库光通信组评估（2026-08-12 /save 挂载 · ⇒ 2026-08-13 问答板裁：入组并打分）**：8/12 CPO 纯度鉴定出纯正 7 只，其中天孚通信 300394、仕佳光子 688313 为库外标的（光通信组现 9 只无天孚，缺口明显）。待做：评估入库（含 `_index/细分环节.json` 映射 + build_index 重建 + 若走 dualscorer 一并打分）；分组改动遵三处同步纪律（json._meta↔模板↔重建推送）。
