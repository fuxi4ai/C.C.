...
- [x] **EAL v3 吸收（shadow.7 · 2026-08-19 VV 交付 · 生产候选 Mac CPython 3.13.3 全套 35/35+19/19+7/7 双模式全绿已验 · 沙箱 3.10 34/35 根因已查实非包缺陷）**：① 吸收执行——backtest/eal_v3/ 平铺落位（import discovery 适配+relocation test）+ Brain canonical（PRD/方法合同）✓；② 资产清单已裁（沿用包内冻结清单：SPY+VIX/CL/DGS2/DGS10/SMH · baseline 自资产 trailing mean）✓；③ 分钟级不纳入已裁 ✓；④ §3 数据迁移全链路完成 ✓（sealed `ac00862e…`→candidate→schema 双跑→intake 27/27→编码→冻结 calendar/config→DAILY_SHADOW 首跑 exit=0→load 候选库 76 行→回读 16 表全对）；⑤ 27 条 legacy 编码 pass 1：3 frozen（FOMC 3/6/7 月）+ 24 exclusion 留 intake（时钟/共识点值/未来事件等 · coding_notes 在盘）。**✅ 升级实施收口（2026-08-19 Doctor「继续，直至升级结束」授权下完成）· 待独立验收：PRD 20 R/N 全部 [ ]（VV/Doctor 落 ✓）· additive schema 生产采用（读者切换）待 Doctor 后续裁定 · 24 条 exclusion 的后续编码（补时钟/共识源）与 27 条全编码随 VV 后续轮**。依据：shadow.7 交接回执 + CC_APPLY.md + §3 实跑证据（PRD §六 变更历史两行）
