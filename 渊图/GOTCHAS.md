...
**硬证据**：全库「图谱ID 指向不存在节点」**39 张，38 张已 `_DEPRECATED_*_20260926.md`，唯此一张仍活跃**；且 `wiki_autogen.py`
只写缺失卡、**从不清理 stale** ⇒ **不自愈**（同 `worldia.md` 判例）。**根因**：先导件漏做「删节点即弃用同名 wiki 卡」收尾步
...
修后全量重扫 **活跃悬空 = 0**。**实施者不自签，落签归 Doctor／未参与实施的指定验收方。**
...
修后全库「props 值含 `**`」＝ **0**，`region` 键值与 props 键集均未动。**实施者（接手场）不自签。**
...
**已修（同批同脚本）**：`rstrip()`，长度 527 → 526（恰少一个半角空格）；修后全库「desc 结尾带空格」＝ **0**。**实施者不自签。**
...
**E · 状态**：🔄 **已修待验**（B① B② B③ 三件均已修；C④⑤ 按 Doctor 2026-09-26 裁定**只登记不追补**，C⑤ 的文件措辞已就地订正）。
**收口判据（机器可复核 · 可重放）**：`outputs/verify_xiazhi_xiandao_replay_20260926.py` **30 项硬检查全过 · exit 0**（修前 24/26 · exit 1）。
**✅ 落签仍归 Doctor 或未参与实施的指定验收方——实施者（接手场）不自签。**
