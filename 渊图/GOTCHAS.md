...
<!-- 2026-06-24 错题本积压复盘：3 条 ⏳ 已消化 →
     ...
     2026-09-14 错题本积压复盘（Doctor「审查并修复」批全案 · 积压口径 3 条=真待修复/待裁）：三条仍开——
     NOTE-20260911-001（价格层 as_of 畸形日期 · 🔄 待修复 · 防再犯已提炼 CLAUDE.md）、
     NOTE-20260911-002（yuantu_scoring 参与结构评分 · 🔄 待修复·方向性归 Doctor 裁 · 防再犯已提炼）、
...
**✅ 落签（未参与实施的审核者 · 独立核验后自主落签）**：独立核验 21 次实跑全过 —— `python3 rules/test_gate18_wiring.py` 实跑 **6/6 PASS（exit=0）**；另在 `/tmp` 隔离树**自造** 5 类违规（未知 scope ／ 七闭集词尾缀变体 `industryy` ／ 同 base 同 scope 多值真冲突 ／ 缺 scope 段 ／ 多锚）× 两条链 = 15 次负向实跑，`kg_merge_safe.py`（`--dry-run` 与实写两态）与 `rules/kg_promote.py` **全部 exit=1、输出含「第18项未过」、真 canonical sha256 前后均 `99e9d579…` 逐字节未变**，且被拦的运行**连 `backups/` 都未创建**（＝闸在任何写盘/备份动作之前的硬证明）；6 次正向（干净图 ＋「同 base 同 scope 同值」对照）全部放行、零误拦；变更面受控 —— `diff kg_merge_safe.py <其 pre_gate18 备份>` 仅 2 处 hunk（`dry_run` 恒 False 订正 ＋ 第 18 项）、`diff rules/kg_promote.py <其 pre_gate18 备份>` 仅 1 处 hunk（第 18 项），两条链均 `import check_caliber_conflicts` 不含 scope 字面量。附注（不改本次判据）：锚键若挂在**边 `properties` ／ 节点顶层字段名 ／ `properties` 内嵌 dict** 三处位置则不在扫描面，两条链均不拦；canonical 现零实例。
**✅ 落签（同日第二次 · 未参与实施的审核者 · 独立核验后自主落签）**：「扩面」独立核验 —— ① **三处旧绕过均已堵**：`check()` 经 `_iter_keys` 递归产出 nodes／edges **各自顶层键 ＋ 其内任意层级 dict／list 的键名**，`_scan` 对节点与边同款调用；本审核者**自造**夹具（非法键 `zzz_caliber_bogusword_2026`，不复用被测方夹具）逐类实跑 —— 边 `properties` ／ 节点自身顶层字段名 ／ `properties` 内嵌一层，检测器各报 1 处（旧版 `check_caliber_conflicts.py.bak_20260926_pre_broaden` 同夹具三处 **viol 全 0**），经 `kg_merge_safe.py` **实写态（不传 `--dry-run`）** 全部 **exit=1 ＋ 输出含「第 18 项未过」＋ 真 canonical sha256 `4e54c9ca…` 前后逐字节未变 ＋ `backups/` 零新增**。② **第四处**：节点／边**之内**未找到（边自身顶层字段名 ／ 属性嵌三层 ／ list 内 dict ／ list 套 list ／ `data_sources[]` 元素键 ／ `_meta` 嵌套 ／「缺 scope 段」落在内嵌层 —— 7 类自造夹具全部拦下）；**但节点／边之外仍余一处**：`rules/kg_promote.py` 用 `shutil.copy(batch, CANON)` **整文件覆盖**，图根层（如 `metadata` 内）的 `_caliber_` 违规键**不进 `check()` 扫描面** ⇒ 隔离树实测**过闸落盘**（`kg_merge_safe` 侧不可达 —— `merge()` 只读四数组、patch 图根键不落图，已实测）。③ 测试：`python3 rules/test_gate18_wiring.py` **12/12 PASS（exit=0）** · `python3 rules/test_check_caliber_conflicts.py` **8/8（exit=0）**。④ 遗留（不改本次判据）：冲突**成组**仍只在 `properties` 顶层下探，同 base 同 scope 分处不同嵌套层不报（docstring 已声明为设计）；上方原「附注（不改本次判据）…三处位置则不在扫描面」**自本轮起作废** —— 三处已堵，仅余图根层一处。

## [NOTE-20260926-002] 干跑（`--dry-run`）若不覆盖主流程，给的是**虚假通过信号**——本场两度咬人

**状态**: ⚠️ 已知风险（使用纪律 · 已被两处实测坐实） **优先级**: 🟡 中

**触发（同一场两次，同一根因）**：
① `kg_merge_safe.py` 原写 `merge(base, patch, dry_run=args.dry_run)`，而 `merge(dry_run=True)` **不返回已应用图**
   （`if not dry_run:` 跳过 append）⇒ 干跑里下游的「同三元组检测」与新增的口径闸**双双空转**
   （实测：干净图 + 非法锚键的干跑里，闸数到「锚键 0」、非法键零报警）。详见同场 `NOTE-20260926-001`。
② `outputs/surgery_lasttwo_stopped_20260926.py` 首版把 `if DRY: return` 放在**合并循环之前** ⇒ 干跑只跑预检、
   **从未跑过合并期的 props 冲突检查**；正式跑才在合并步炸（`company_ShinEtsuChemical` 的 `region` 冲突，
   因豁免表漏了 `region`）。**干跑当时报的是「通过」。**

**规则**：
- **干跑必须跑完整条主流程**，只在「写盘」这一步分叉；**不得**在主流程中途 return。
- 实现手法（本场采用，可照抄）：干跑前先 `copy.deepcopy()` 一份，在**副本上完整预演**（含合并/边处理/复校），
  有炸就当场报「预演即炸，正式必炸」，再把副本丢掉、不写盘。
- **反面判据**：如果一个 `--dry-run` 的返回点早于任何有副作用的计算，那它**证明不了任何事** ——
  它只证明「预检能过」。

**判别动作（可直接照用）**：写干跑分支时自问一句 —— **「把主流程里某个必然失败的条件故意造出来，干跑会不会报？」**
不会报 ⇒ 干跑的覆盖面有洞。本场复验方正是用这一手（还原旧豁免表 + 干跑）复现了盲区。

**来源**: 2026-09-26 场（`NOTE-20260926-001` 的 `dry_run` 订正 ＋ `surgery_lasttwo_stopped_20260926.py` 首跑自伤 · 两者均由未参与实施的审核者独立复现）
...
**追记 2026-09-18（同根复发 · 第 2 次 · 形态升级：CC 主动把 ignore 路径写进 commit 命令）**：/save 场 CC 给了 `行业研究` 仓的 commit 命令，把 `raw/核实/2026-09-18-高盛中国半导体调研核实札记.md` 与 `outputs/gs/{px_raw.json,px_hk.json}` 列进 `git add`。Doctor 终端实跑 → **git 直接拒绝**（`The following paths are ignored...: outputs / raw`）→ 该仓 commit **未生成**、push 空转（`Everything up-to-date`）；三件至今未入版本控制。**硬证据**：Doctor 终端回执原文（git hint 逐字）· 本仓 `.gitignore` 两处显式条目（`raw/` 注「研报源文件（版权敏感 + 体积大）」· `outputs/`）· `行业研究/CLAUDE.md` L207 明文「⚠ outputs/ 整目录被 gitignore——…**别列进 add 清单**」· `.git/index` 实测 `raw/` 与 `outputs/` 命中数 **0/0**（从未被跟踪）。**根因**：不是不知道坑，是**规矩停在「文档 + 人脑」两层**——FIX-20260617-001 已立、CLAUDE.md 已写，但 CC 起草命令时**既不读目标仓 `.gitignore`，也不回读该仓 CLAUDE.md 的 git 段**；`brain-save` Step 5 的 v2.9 只规定「只加本次范围内文件」，未规定「加之前先判该路径是否被 ignore」。**影响面**：本次零损失（未误加、未误 commit）；但形态比第 1 次更重——**第 1 次是自动流程撞墙，这次是人（CC）主动把 ignore 路径写成命令**，说明「文档层规矩」对生成式下游不生效。**建议修法（候选 · 待 Doctor 裁）**：① `brain-save` Step 5 增硬闸——列 `add` 清单前先 `git check-ignore -v <paths>`（或用 `find`/读 `.gitignore` 手判），命中即从清单剔除并改报「该产物按仓规 local-only」；② 各仓 `CLAUDE.md` 的 git 段前置于文件头部（现 L207 埋在示例注释里，起草时不会翻到）。**预防门禁**：凡 CC 代拟的 commit 命令，**清单里每个路径都必须先过一遍目标仓的 ignore 判据**——这不是「要不要严谨」的取舍，是命令生成前的必过步。**状态**：🔄 已修待验（追记已落；门禁实装待 Doctor 批）。**来源**：2026-09-18 晚场 /save（Doctor 终端回执）· logs/2026-09-18-高盛半导体调研核实与股价归因.md
...
**追记（2026-09-19 · 同根复发第 2 例 → 按合同⑤登记「应升格通用教训」，不由实施者闭环）**
**触发**: Doctor 提交一张网传「大摩 Nvidia NVL72 Bill of Materials（Exhibit 3）」图，问大摩近期研报是否真有此表。核实本体结论：**表为真**（摩根士丹利 2026-05-20 研报《亚太大中华科技硬件：Rubin机架物料清单、组件含量及ODM附加值分析》·17 页 · Exhibit 3），**最早公开出现 2026-05-21**（wccftech `datePublished 2026-05-21T05:15:16Z`）。8 月下旬至 9 月中旬的「大摩近期研报」二次流传，触发点＝彭博 2026-08-22 报道英伟达通知客户 2027 年初出货机型涨价 >15%，各家转载一律写「此前估算」。
**复发点（本库侧）**: `product_NvidiaVR200Rack` / `product_NvidiaGB300Rack` / `product_HBM4ForVR200` 三个节点的 `data_sources` 记的是**转述源日期**——`reference: "Research, 2026-08-16"` · `data_vintage: 2026-08-16` · `file: 2026.08.16-Boss老白-英伟达涨一倍_…md`（抖音转录）。**底层报告的时点（2026-05-20）在整个 provenance 里不存在**。下游若按 `data_vintage` 判新鲜度，会把 5 月口径读成 8 月新料——这与全网那次误读是**同一个坑，只是换在 canonical 自己的字段上**。
**同链第二处损耗（量词/精度）**: 转述把精确值抹成概数——$7,803,148→7,800,000 · $3,994,551→3,990,000 · $2,001,600→2,000,000 · $373,939→370,000 · $116,730→117,000 · $72,080→72,000 · $2,520,000→2,500,000。原表 12 行 Diff% 与两列合计**内部自洽**（重算：GB300 3,994,551 / VR200 7,803,148 / 总 +95.34%），概数化后仍自洽，故**不触发任何既有 QA 闸**。
**复发形态归纳**: 第 1 例丢的是**正文限定词**（「年内」），本例丢的是 **provenance 层的原报告时点**——同一根因「转述链不携带时点」，换了载体即绕过了既有信源定级规则（该规则只管正文，不管 `data_sources` 字段）。
...
**状态**: ⚠️ 已知风险（2026-09-14 状态词归一·原「🟢 已知边界」不在计数闭集） **优先级**: 🟡 中
...
**追记 2026-09-18（独立验收 · subagent 实跑实读）**：未参与实施的独立验收员出具 **PASS**——代码实读（函数+两调用点）/单测 5/5 实跑/mutation 负向验证（桩死检测器→回归测试立即炸·fail-fast 成立）/canonical+批前备份双 base 全量回放 3/3 命中+0 误报/py_compile exit 0/治理留痕核对无夸大。轻微发现 4 条（装饰性 docstring 已修/字面歧义注记/_nrm 冗余字符无害/变异测试顺序巧合 fail-fast）均不构成 FAIL。✅ 已落签（2026-09-18 · 独立验收 PASS · 代签者=独立验收 subagent · authority=Doctor 明示『不需要我核了吧』）
...
**状态**：🔄 已修待验（patch 已备、随 promote 落地；签字按分轨（事务性的由未参与实施的 subagent 代签）✅）。**第 11 例 · 应升格通用教训**——升格仍归 Doctor 裁。
...
**状态**：⚠️ 已知风险（**部分已修 · 待 Doctor 裁处置方向**）。**⚠ 2026-09-26 订正**：本行原写「已登记未修」——**字面已陈旧**：第 12 例点名的**长芯博创簇已合并**、且同日另处置**高置信三组**（台玻/沃尔德/胜宏），**同类违例只减不增**（承落签审核者独立复算）；**余 45 簇待裁**，故仍为 ⚠️ 而非 ✅。**第 12 例 · 应升格通用教训已 4 次登记**——**2026-09-26 Doctor 已裁「升格」→ 见 [[通用教训]] `G-X198`**。
**来源**：2026-09-26 场（渊图两条 promote 执行中撞见 · 脚本 `outputs/` 内联粗扫 · 扫描时点 canonical sha `6983ea52…`＝**改前**态；对应 TODО 已发布的 72 组数字）
**✅ 落签（未参与实施的审核者 · 独立核验后自主落签）**：独立复现「手术变更面受控 ＋ 墓碑整节点在场」两项 —— ① 变更面：`backups/行业知识图谱_完整数据库.json.bak_surgery_changxinbochuang_20260926_042626` ↔ canonical 逐节点/逐边 dict diff ＝ **删 1 节点**（`company_BochuangTech`）· **改 2 节点**（`company_ChangXinBoChuang` ＋ `company_Changxincheng`）· **增 1 边**（`rel_company_Changxincheng_company_ChangXinBoChuang` · `part_of`），**删边 0 ／ 改边 0**，其余 6481 节点与 7109 边**逐 dict 全等**（6484/7109 → 6483/7110，`metadata` 两计数与数组实测一致）；`stock_code 300548.SZ` 确已迁至主节点，长芯盛 `aliases` 去「长芯博创」后为 `["长芯盛"]`。② 墓碑：`mapping/_tombstones/2026-09-26_changxinbochuang_merge.json` 的 `deleted_node_full` 与术前备份该节点 **16 字段逐字段 dict 全等**（含 `source_span`/`_code_correction_20260924`），**可单独复原**。③ 附带：收尾 followup 只挪 `metadata.total_nodes/total_edges` 两计数，节点与边数组逐字节无差；`rules/check_desc_shrink.py <该术前备份> <canonical>` **exit=0**（desc 缩减 0 处）。
...
**✅ 落签（未参与实施的审核者 · 独立核验后自主落签）**：独立复现「手术变更面受控 ＋ 墓碑整节点在场 ＋ 同类违例只减不增」三项 —— ① 变更面：`backups/行业知识图谱_完整数据库.json.bak_surgery_samename_20260926_044624` ↔ canonical 逐节点／逐边 dict diff ＝ **删 2 节点**（`company_TaiwanGlass` ／ `company_Worldia`）· **改 3 节点**（`company_Taibo` ／ `company_WorldDiamond` ／ `company_VictoryGiant`）· **改 2 边**（两条 `rel_Worldia_supplies_*`：`source` 重指、**边 id 留旧**、其余字段全等）· **增边／删边均 0** · 顶层键与 `report_meta` 无夹带；两被并节点 props **逐键一致**并入（含 G2 `stock_code 688028.SH` 迁至 survivor）、aliases **全 token 覆盖**（判据：出现于 survivor.aliases **或** 等于 survivor.name）、desc 子串并入 ／ data_sources 全在场；G3 胜宏 aliases 去 `Shenghong`，**盛弘 `company_Shenghonggufen` 逐字段未动**；结构不变量（重复 id ／ 悬挂 ／ 自环 ／ 非法 type ／ 同三元组 ／ 边 id 重复）**全 0**；`metadata.total_nodes/total_edges` ＝ **6481/7110 与实测一致**，`_meta_surgery_samename_20260926` 在场且其 backup／tombstone 两路径实存。② 墓碑：`mapping/_tombstones/2026-09-26_same_name_highconf_merges.json` 两被删节点**整节点原文**（对拍术前备份**逐字节等价**，G2 含 `_meta.公司品格典范_2026_05_28`）＋ `edges_retargeted` 2 条 ＋ `alias_removed.before` 七元列表（与术前一致）—— **可单独复原**。③ 只减不增（本审核者自写脚本复算）：「alias 等于另一节点 name」**68 → 67**（差集**恰好**为 `company_Worldia` 1 对，零新增）· 同名 token 组（name＋aliases）**337 → 333**（消失组恰为 `台玻`／`沃尔德`／`688028.sh`／`shenghong`，零新增）· company 限定同名组 **71 → 67**；`rules/check_caliber_conflicts.py` **exit=0** · `rules/bare_alias_check.py` **exit=0** · `rules/check_desc_shrink.py <术前备份> <canonical>` **exit=0（0 处）**。④ 附注（不改本次判据）：被并节点 `company_Worldia._meta`（公司品格典范条目）**未并入 survivor**（survivor 无 `_meta`），而并入的 desc 仍写「（详见 _meta）」⇒ 图内该指针悬空，原文仅存墓碑（可复原）；另本条 ③ 的「60 → 59」与 [[通用教训]] `G-X198` 的「60 处」本审核者**仅在全 CJK 别名口径下复现**（同态 6484／6483 为 60／59；不限 CJK 则 69／68／67），该数位口径未写明。

**✅ 落签（未参与实施的审核者 · 独立核验后自主落签）**：同名实体三批处置独立核验 **PASS**（P1–P9 逐条成立，边界见末）—— ① **终态与基线**：canonical 实测 **6456 节点 / 7118 边**，`metadata.total_nodes/total_edges` 与两数组实测逐数相等，`last_op=surgery_batch3c_lasttwo_20260926`，五条 `_meta_surgery_*`（batch1／batch2／batch3／batch3b／batch3c）记账全在场；**开工前基线以内容判定**（不采文件名时间戳——`shutil.copy2` 保源 mtime 致错位，实测批1备份 mtime 04:52 而文件名戳 05:43）＝ `backups/…bak_surgery_batch1_20260926_054320` 的 **6481 / 7110**（其 `metadata.last_op=surgery_samename_highconf_20260926`）⇒ 全程 **−25 节点 / +8 边**。② **逐批变更面**（逐节点／逐边 dict diff ＋ 本审核者独立重实现五批算法正向回放，均与实测后态**零差异、边序亦同**）：批1 **删18／改18 节点 · 19 条边原地重指（id 留旧）· 边数不变**；批2 **节点数不变／改20 · +9 条 `part_of`**（C/D 去错挂别名 9 处 · Coherent↔Finisar 只标注）；批3 **删7／改8 · 删1 边**（18 条重指 ＋ `rel_company_CXMT_Storage_concept_CXMTIDMModel` 撞同三元组按判例删新，旧边 `rel_CXMTIDM_Model` 在场）；批3b／批3c **各改3 · 增删各 0**；**零夹带**（顶层键集 · `report_meta` · 节点相对序 · 边序全未动）。③ **字段保全**：批1 的 18 个与批3 的 7 个被并节点**全部顶层键**按去向并入（`properties`／`_meta` 合并路径实跑通过）。**【⚠ 2026-09-26 深夜订正 · 承接手场复验】** 本句「**全部顶层键**」经复算**与实测不符**：09 月墓碑 **33 条**「有 survivor ＋ 整节点」记录中 **7 条有缺口**，且全在 09-26 当日批次——**含批 1 的 4 条**：`company_AOI` 缺 `created_by` · `company_DiscoCorporation`／`company_Risonoko` 缺 `enrichment_source` · `company_SumitomoElectric` 缺 `source_span`；另批 3 的 `company_ShengmeiShanghai` 缺 `enrichment_source` · 高置信组的 `company_Worldia` 缺 `trend` · 先导件的 `company_GuangdongFirstRare` 缺 `source_span` ＋ `enrichment_source`。宜读作「**properties／desc／aliases／data_sources 四路并入，其余顶层键按需、有缺口**」。**⚠ 定性＝文档措辞失准，非数据丢失**——被并节点**整节点原文全在墓碑**（本条早前落签已复验逐字段全等），可逐字复原；**Doctor 2026-09-26 裁「只登记＋订正措辞、不从墓碑追补」**（理由：值未丢、追补要二次动已过评审的生产图、收益是字段完整性非信息完整性）。详见本条末尾「**追记 2026-09-26 深夜**」。；**全库「desc 写『详见 _meta』而 `_meta` 不在」＝ 0 处**（唯一提及者 `company_WorldDiamond` 带 `_meta`——早前 Worldia 悬空指针事故**已修复**）。④ **墓碑**：批1／批3 墓碑内 **25 个被删节点整节点原文与术前备份逐字段全等** ⇒ **节点层可单独复原**。⑤ **剩余簇**：自写连通分量复算 company 域 **恰好 5 簇**——先导系(4) · 厦芝(3) · Coherent/Finisar · 中芯国际 · 信越化学，与预期一致、**无多出簇**。⑥ **停手项**：`company_NanjingSMIC`／`company_ShinEtsuChemical` 与术前**逐字段全等**、`region` 仍「中」、`updated_at` 未动、未进任何 merged 名单（仅在 `stopped_on_prop_conflict`）。⑦ **改名留痕**：5 个改 `name` 节点全带 `_meta.name_before_20260926` 且其值＝实读改前名。⑧ **新工具** `rules/scan_existing_name_collisions.py` **只报不拦三项 exit 0**（无参／`--only A`／`--only D`）· 跑前后 canonical **sha256 不变**（`0dea7fca…`）· **A/B/C/D 四档强制分档且每簇带理由** · 本审核者自造含已知 A 类碰撞的临时图**实判 A 档**。⑨ **QA 四件真实 exit code 全 0**（`check_caliber_conflicts` · `bare_alias_check` · `check_desc_shrink <批1术前备份> <canonical>`「0 处」· `test_gate18_wiring` 16/16）＋ 自算不变量**全 0**（悬挂／自环／非法 type／同三元组／边 id 重复／节点 id 重复）。**边界（不构成 FAIL · 供后手）**：(a) `edges_retargeted` 仅存**边 id**、批3 那条「撞三元组删」的边其 `description`／`weight`／`evidence` **只存 backups** ⇒「不依赖 backups 复原」在**节点层成立、边层不成立**；(b) 批3b／批3c **未落墓碑文件**（`before` 值记于 canonical 的 `_meta_surgery_batch3b/3c`，仍可复原）；(c) 新工具 A 档「共享 `stock_code`」判据**只对已共享 token 的节点对可达**——同码异名且不共享 token 者**不进候选**（今日全库重复 code 实测 **0 组**，无实害）；(d) **派生层未刷**：6 张 orphan wiki 卡（`changchuan`／`changfeifiber`／`changxinstorage`／`shenmeishanghai`／`cxmt_storage`／`sumitomoelectric`）其 `图谱ID` 指向已删 id，survivor 卡同时在。

**✅ 落签（未参与实施的审核者 · 独立核验后自主落签）**：三批处置收尾复议三项独立核验（＋ canonical 零触碰）—— ① **F1 成立**：`wiki/worldia.md` 弃用改名——原名已消／新名在场（`shutil.copy2`+`unlink`，**保源 mtime** 故「文件日期」不可作改名时点证据）· **内容逐字节一致**（基线＝本审核者自读 `.git/objects`（pack 解析＋自校 sha1）取 HEAD 提交内 `wiki/worldia.md` blob `b3760d8d…`，实读 sha256 `bfa83bf4…` 两侧相等 **`cmp` 零差异字节**；**局限**：无 `.bak`，该基线只证「＝最后提交版」——supplement：index 内该条目 stat（size 2516／mtime 09-11 04:26:45Z）与现名文件实测逐项相同 ⇒ 提交后至改名前无中间态）· 幸存 `worlddiamond.md` 在盘 · **重跑悬空扫描：活跃卡悬空 0**（上一轮的 1 已消；把该卡按活跃计重建 ⇒ **复现上轮那 1**，口径吻合）；② **F2 成立**：两条不实解释**已撤**（全文仅于撤回声明内以引文出现并标「不实」；其支撑事实「A 段无 #06／四段 44 簇号零重复／#06#24 不在 A、C 段」本审核者自数复核成立）·「表体互异 44 个簇号恰缺 #17/#35」自数成立 · 基线「6484/7109 · company 913 · sha `266946a6…`」与 `backups/…bak_surgery_changxinbochuang_20260926_042626` 实测**逐数全等** ·「本件写于复分类之前」有表体旁证（#14 长芯博创仍记待处置，而该簇 04:26 已合并；`surgery_batch1` docstring 记板上 A 列 **20** 簇 vs 本件 A 表 **21** 行）·「**42 无独立于实施方的出处**」成立 ·「44/46/42 未闭合」成立（修订记录 `21+12+8+2+3＝46` 与表体 `21/12/9/2＝44` 互斥、且 A 表 21 行不含 #08 而修订记录称 A「实为 21」）；③ **F3 成立**：`norm_code()` 四测——异形后缀同码·零共享 token **落 A 档**（同一合成图在**还原旧逻辑的对照版下 0 簇** ⇒ 盲区确已合上）· 占位符 `-`／`N/A`／`—`／空串**不并簇** · 错码同码对**确会落 A**（假阳性风险已如实标）· 真图 **6 簇 A3／B1／C1／D1**，多出那簇 `company_MetaX`／`company_MuxiCo` 经 name（沐曦／沐曦集成 vs 沐曦股份）＋aliases＋props＋desc（同为国产 GPU 四小龙·CUDA 兼容 6000+ 应用）＋ data_sources ＋ 09-24 代码底稿（`company_MuxiCo: 688802.SH`）核实为**同一上市主体 688802**，非仅凭 code 相等 · 无参／`--only A`／`--only D` 三形态 **exit 0 且 canonical sha256 不变**；④ **F4 成立**：canonical **6456/7118** 与 `metadata` 两计数一致、全程 sha256 `0dea7fca…` **不变**，`check_caliber_conflicts`／`bare_alias_check`／`test_gate18_wiring` **真实 exit 0**（真冲突 0 组／0／16 项全过）。**边界（不构成 FAIL · 供后手）**：(a)「42 只见于…两处」按「本件之外」口径成立（问答板 `outputs/问答板_2026-09-26-0525.html` ＋ `rules/scan_existing_name_collisions.py`，均自产），**计入本件自身则为三处**、宜加限定；(b) 工具 docstring「重复 code **0 组**（零实害）」系归一前**原样口径**，按新语义实测 **1 组**＝本修复命中的那一簇；(c) 印刷标签「token 命中组 N」现把 code 分组一并计入（真图实为 token 组 7＋code 组 1）；(d) 首版节标题「20/10/9/2 簇」**盘上无基线可核 ＝ 未核**（A＝20 有修订记录旁证，B＝10 无旁证）；(e) 新 A 档簇 `MetaX`／`MuxiCo` 系本工具**新暴露、不在 42／46 簇授权面内**，且 props `region` 冲突（「其他」vs「中」）触批1「冲突即停手」判例 ⇒ 处置权仍归 Doctor。

**✅ 落签（未参与实施的审核者 · 独立核验后自主落签）**：末批三簇（沐曦／信越／中芯）与词表补录独立核验 **PASS_WITH_LIMITS**（P1–P9 逐条成立；边界见末，不构成 FAIL）—— ① **术前态以内容判定**（`copy2` 保源 mtime ⇒ 文件名戳不可作序）：沐曦术前＝`backups/…bak_surgery_muxi_20260926_062452`（`metadata.last_op=surgery_batch3c_lasttwo_20260926`·6456/7118）· 信越/中芯术前＝`…bak_surgery_lasttwo_20260926_062657`（`last_op=surgery_muxi_merge_20260926`·6455/7118）；且 **062619 与 062657 逐字节相同** ⇒ 首跑确未写盘（只余孤儿备份）。② **逐簇 dict diff 零夹带**：沐曦 删 1（`company_MuxiCo`）／改 1（`company_MetaX`）／改 2 边（两 `rel_*MuxiCo*` 重指·id 留旧）；信越+中芯 删 2（`company_ShinEtsuChemical`／`company_NanjingSMIC`）／改 2（`company_ShinEtsu`／`company_SMIC`）／改 2 边／删 1 边；顶层键集·节点与边**相对序**·`report_meta` 全未动，`metadata` 只动 `last_op`／两计数／两条 `_meta_surgery`。③ **region 三项语义正确**：`company_MetaX`＝**中**／`manual_20260926`（非 kw/en）· `company_ShinEtsu`＝**其他**／`kw/en`（并入方占位「中/默认」**未覆盖**）· `company_SMIC` `港/code`→**中/`manual_20260926`** 且 `stock_code=688981.SH` 确已补（术前该键缺，实测）。④ **边**：三簇共**重指 4**／**删 1**；被删 `rel_NataOpto_Supplies_SMIC` 重指后三元组 `(company_NataOpto, company_SMIC, supplies)` 与幸存方既有 `rel_NataOpto_SMIC_ArFsupply` **确同**，整边原文与术前备份**逐字节等价**入墓碑 `dropped_edge_full`。⑤ **墓碑可单独复原**：三被删节点整节点原文与术前**逐字段全等** ＋ region 前后值（`region_changed`／`region_rulings`）⇒ 节点层与删边层不依赖 backups；重指边原端侧可由「后态恰一端＝幸存方且无自环」唯一推定。⑥ **词表补录**：`FX_KW` 实含「信越」「Shin-Etsu」，与 `.bak_pre_shinetsu_20260926_062657` 差异**恰为该一行**；自造节点（name 信越化学／aliases `['Shin-Etsu Chemical','信越']`／无 code）**补录后 `('其他','kw/en')`、还原旧词表 `('中','默认')`**；**旁伤 0**——882 个 company 节点新旧词表逐点对判**零变化**。⑦ **终态簇**：自写连通分量（token 归一 ∪ code 归一）复算**恰好 3 簇**＝先导系(4)／厦芝(3)／Coherent·Finisar，**A 档「真错」＝0 簇**（与工具自报同数）。⑧ **不变量与记账**：悬挂／自环／非法 type／同三元组／边 id 重复／节点 id 重复**全 0**；`metadata.total_nodes/total_edges`＝**6453/7117** 与实算一致；`_meta_surgery_*` 共 **10** 条（前轮 7 条全在＋末批 `muxi`／`lasttwo_stopped`＋旧 `lanqi_20260912`），各 `backup`／`tombstone` 路径实存；「desc 写『详见 _meta』而 `_meta` 不在」**0 处**。⑨ **真实 exit code**：`check_caliber_conflicts`／`bare_alias_check`／`check_desc_shrink <末批两术前备份>`（0 处）／`test_gate18_wiring`（16/16）**全 0**；`scan_existing_name_collisions` 三形态（无参／`--only A`／`--only D`）**全 0**，9 次运行前后 `mapping`／`rules`／`outputs`／`backups` **零文件变化**、canonical sha256 **`bb93652d…` 不变**。⑩ **自伤留痕如实**（`/tmp` 沙箱内还原首版 `PROP_NO_CONFLICT={"…_region_src"}` 实测）：正式跑**复现**「`❌ company_ShinEtsuChemical 的 properties.region 冲突——已备份，未写盘。`」＝脚本注「合并步才炸」逐字吻合；**修正版干跑会报**（exit 1·「预演即炸」）、干净图干跑 exit 0 ⇒ 干跑确已覆盖合并循环。**边界（不构成 FAIL · 供后手）**：(a) **沐曦件的干跑仍不覆盖合并循环**——沙箱实测：去掉其「A · region 先订正」后干跑 **exit 0 放行**、正式跑才炸（同类盲区本次只修 `lasttwo` 一件；本场无实害，该件跑通）；(b) 该首跑自伤**只见于脚本注释**，`GOTCHAS` 与 canonical 记账均未登记；(c) `metadata['nodes']/['edges']＝5375/5996` 系**末批之前即已陈旧**的旧式计数（与 `total_*` 并存，末批未动）；(d) 被删边 evidence 句（「南大光电28nm光刻胶导入中芯国际产线」）与幸存边不同 ⇒ 该句只存墓碑。

**追记 2026-09-26 深夜（接手场 · 厦芝／先导两件合并的独立重放 ＋ 三项实错登记）**:

**A · 两件合并的事实（独立重放 · 可重放脚本已入仓 · 不采信转述）**：`surgery_xiazhi_merge_20260926`（06:39）与
`surgery_xiandao_merge_20260926`（06:41）已落盘，**6453/7117 → 6451/7117 → 6449/7118**。
接手方（未参与实施）以术前备份逐节点／逐边 dict diff 重放：**变更面与脚本声明逐条一致、零夹带**
（顶层键集 · `report_meta` · 节点与边相对序全未动）；**4 个被并节点墓碑整节点原文 vs 术前备份逐字段全等** ⇒ 节点层可单独复原；
结构不变量（重复 id／悬挂／自环／同三元组／边 id 重复／空 type）**全 0**；`metadata.total_*` 与数组实测一致；
12 条 `_meta_surgery_*` 的 backup／tombstone 路径**全部实存**；`data_sources` 并集**无漏**；region 与幸存方**无冲突、无静默丢失**；
**company 域连通分量复算余 1 簇 ＝ `Coherent`／`Finisar`（有意的并购别名）⇒ A 档「真错」＝ 0 簇**。
QA 四件真实 exit 0（`check_caliber_conflicts`／`bare_alias_check`／`scan_existing_name_collisions`／`test_gate18_wiring` 16/16）。
**复验脚本 `outputs/verify_xiazhi_xiandao_replay_20260926.py`（只读·仓根直接可跑）——接手方首跑 26 项硬检查 24 过 / 2 败（败者即下文 B② B③）；三件修完后为 30 项全过 · exit 0。
⚠ 可重放的**前提是同仓本地有三件备份**（`backups/…bak_surgery_xiazhi_20260926_063930` ／ `…bak_surgery_xiandao_20260926_064103` ／ `…bak_fmtfix_20260926_070450`）——而 `backups/` 与 `outputs/` **均在 `.gitignore` 内**（本仓设计如此：本地维护目录不入版本）。⇒ **脚本与备份都只在本机，克隆仓库拿不到**；异地复现须先取得这三件备份。**登记在案，不粉饰。**

**B · 三项实错（均由本件引入 · 接手方独立复现）**：
① 🔄 **已修待验** — **`wiki/firstrarematerials.md` 活跃悬空卡**：其 `图谱ID=company_FirstRareMaterials` 已被 06:41 并入，卡未随弃用。
**硬证据**：全库「图谱ID 指向不存在节点」**39 张，其中 38 张在本件处置前已弃用改名、唯此一张仍活跃**；且 `wiki_autogen.py`
只写缺失卡、**从不清理 stale** ⇒ **不自愈**（同 `worldia.md` 判例）。**【⚠ 措辞订正 2026-09-26 接力 · 承未参与实施的核验者边界 (b) 与接手场自算】** 本句原写「38 张已 `_DEPRECATED_*_20260926.md`」——**过宽**：40 张 `_DEPRECATED_` 卡按各自处置日后缀实为 **`_20260910`×28 ／ `_20260926`×9 ／ `_20260823`×1 ／ `_20260828`×1**，**通配 `*_20260926.md` 只对 9 张成立**；且其来源非「早前三件收尾脚本」一处（其中 28 张系更早批次 09-10 留存）。**事实核心（39 悬空／38 已弃用／1 活跃、不自愈）不受影响。**
**根因**：先导件漏做「删节点即弃用同名 wiki 卡」收尾步
（该步当日已在 `cc_fix_batch_wrapup_20260926.py` 6 张 ＋ `cc_fix_wrapup2_20260926.py` 1 张上固化）。**已修**：
`outputs/cc_fix_firstrare_wiki_deprecate_20260926.py`（改名不删 · 内容逐字节一致 913 B · 幸存方卡 `xiandaogroup.md` 在场）；
修后全量重扫 **活跃悬空 = 0**。**签字按分轨：事务性／事实性的由未参与实施的 subagent 代签；方向性的问 Doctor（接手场为实施方，故由未参与实施的 subagent 代签）。**
② 🔄 **已修待验** — **`company_Xiazhi.high_ar_drill_monthly_output_2026_07_highAR` 的值含 markdown `**`**
（`"数万-十几万支（**高长径比「长刃」细分**口径；当时仍在认证 · 源 2026-07-08）"`）。**硬证据**：全库 6449 节点中
**props 值含 `**` 的节点数 ＝ 1**（仅此一个），而同场 **48 个节点的 `**` 全部只在 `description` 里** ⇒
本库口径是「desc 允许 markdown、props 值不允许」，本件越界。**根因**：`surgery_xiazhi_merge_20260926.py` 的 `PROP_REMAP`
把口径括注与展示层加粗一并写进了数据字段。**已修（Doctor 2026-09-26 裁「两项都修」）**：
`outputs/cc_fix_xiazhi_xiandao_format_20260926.py` —— 值内 `**` 去壳、**文字一字不减**（长度 −4 ＝ 恰删两对星号）；
修后全库「props 值含 `**`」＝ **0**，`region` 键值与 props 键集均未动。**（接手场为实施方）签字按分轨：事务性／事实性的由未参与实施的 subagent 代签；方向性的问 Doctor。**
③ 🔄 **已修待验** — **`company_XiandaoGroup.description` 结尾带一个半角空格**。**硬证据**：全库 desc 结尾带空格的节点数 ＝ 1（仅此一个）；
术前备份该项结尾为「…全产业链。」无空格 ⇒ **本件引入**。**根因**：`surgery_xiandao_merge_20260926.py` 的 `DESC_ADD` 字面量以「。 」收尾。
**已修（同批同脚本）**：`rstrip()`，长度 527 → 526（恰少一个半角空格）；修后全库「desc 结尾带空格」＝ **0**。**签字按分轨：事务性／事实性的由未参与实施的 subagent 代签；方向性的问 Doctor。**

**C · 两项边界（本场批量性 · 非末件独有 · 供后手）**：
④ ⚠️ **边的字段残缺是「存量性不一致」，非本场引入**（**接手方初判为「本场 11 条新边缺字段」，二轮复算后自我纠偏**）：本场新挂的 **11 条 `part_of` 边**
（批 2 的 9 条 ＋ 长芯博创 1 条 ＋ 先导 1 条）字段为整齐 7 个（id／source／target／type／description／data_sources／updated_at）。**但同型边缘在全库早已存在**：
`created_at` 全库缺 **82** 条（含 `part_of` 类 6 条）、`evidence` 缺 23 条 `part_of`（新边仅占 11）· `weight` 缺 13 条 `part_of`（新边占 11）· `created_by` 缺 17 条（新边占 11）·
`direction` 缺 16 条（新边占 11，存量取值为 `directed` 731/749）。⇒ **本场 11 条只是加入存量少数派**，宜按「全库边字段归一」统一处置，**不宜只补这 11 条**。
（下游实读：`wiki_autogen.py` 依 `e.get("weight")` 排序与展示 ⇒ 缺 `weight` 会使该边在卡内排序垫底、权重列空；`consumers/龙鱼五力/check_ds_evidence.py` 读 `e.get("evidence")`。）
**未处置——待裁。**
⑤ ⚠️ **被并节点顶层键并入不全，且早前批次亦有**：09 月墓碑 **33 条**「有 survivor ＋ 整节点」记录中 **7 条有缺口**
（`source_span`×2／`enrichment_source`×4／`created_by`×1／`trend`×1），其中 **批 1 已占 4 条**
（`company_AOI` 缺 `created_by` · `company_DiscoCorporation` ／ `company_Risonoko` 缺 `enrichment_source` · `company_SumitomoElectric` 缺 `source_span`）。
⇒ 本文件早前落签句中「批 1 的 18 个与批 3 的 7 个被并节点**全部顶层键**按去向并入」**与实测不符**，
宜读作「properties／desc／aliases／data_sources 四路并入，**其余顶层键按需、有缺口**」。**已处置（Doctor 2026-09-26 裁「只登记＋订正 GOTCHAS 措辞、不从墓碑追补」）**：本文件早前落签句已**就地加 ⚠ 订正块**；**canonical 未动**（值未丢，整节点原文全在墓碑）。

**D · 陈旧计数订正**：本条第 12 例正文「**余 45 簇待裁**」**已陈旧**——同日各批后 **company 域残余重复簇 ＝ 1**
（`Coherent`／`Finisar`，有意共享），**A 档真错 ＝ 0**；接手方以自写连通分量（token 归一 ∪ code 归一）独立复算，与 `scan_existing_name_collisions.py` 自报同数（A＝0／D＝0）。
**另**：接手场**无法读取**前一会话（会话名「todo处理」· `local_2d73f575`）的上下文——该会话因超 1M token 上限每轮 `400` 中断，
其独立复验所报「**三项不成立**」的**原文未能取得**。**本追记 B 组三项系接手方自行重放后独立得出，与那三项是否同一批无从核对**——如实登记，不冒充复原。

**E · 状态**：✅ 已验收（PASS_WITH_LIMITS · 落签者＝未参与实施的独立核验 subagent · authority＝Doctor 2026-09-26「事务性的/事实性的 subagent 代签」）（B① B② B③ 三件均已修；C④⑤ 按 Doctor 2026-09-26 裁定**只登记不追补**，C⑤ 的文件措辞已就地订正）。
**收口判据（机器可复核 · 可重放）**：`outputs/verify_xiazhi_xiandao_replay_20260926.py` **30 项硬检查全过 · exit 0**（修前 24/26 · exit 1）。
**✅ 落签按分轨：事务性／事实性的由未参与实施的 subagent 代签；方向性的问 Doctor（接手场为实施方，故由未参与实施的 subagent 代签）。**
**来源**：2026-09-26 接手场（`local_2d73f575` 超限后接手）· 复验脚本 `outputs/verify_xiazhi_xiandao_replay_20260926.py` ·
格式归正 `outputs/cc_fix_xiazhi_xiandao_format_20260926.py`（备份 `backups/…bak_fmtfix_20260926_070450`）·
wiki 弃用 `outputs/cc_fix_firstrare_wiki_deprecate_20260926.py` ·
术前备份 `backups/…bak_surgery_xiazhi_20260926_063930` ／ `…bak_surgery_xiandao_20260926_064103`

**✅ 落签（未参与实施的审核者 · 独立核验后自主落签）**：厦芝／先导两件 ＋ 格式归正 ＋ wiki 弃用独立核验 **PASS_WITH_LIMITS**（A1–A12 逐条成立；边界见末，不构成 FAIL）。**纪律**：不采信任何转述／脚本注释／任务书描述，全部以自读自跑为准；未运行任何 git 子命令；除本段与状态行外未写盘（落签后回核：仓内 7477 文件 mtime／size 指纹与核验前逐行相同）。① **A1 成立**：P1（`…bak_surgery_xiazhi_20260926_063930`，自读 `metadata.last_op=surgery_lasttwo_stopped_20260926`）↔ P2（`…bak_surgery_xiandao_20260926_064103`，`last_op=surgery_xiazhi_merge_20260926`）自写逐节点／逐边 dict diff ＝ **删 2**（`company_XiaSi`／`company_XiamenXiazhi`）· **改 1**（`company_Xiazhi`：aliases＋data_sources＋description＋properties＋updated_at）· **边与边序零变化**；增节点／增边／删边／改边**均 0**；顶层键集 · `report_meta` · 节点与边相对序全未动（`metadata` 仅增 `_meta_surgery_xiazhi_20260926` 与 `last_op`／`total_nodes` 记账）⇒ **零夹带**。② **A2 成立**：P2↔P3（`…bak_fmtfix_20260926_070450`，`last_op=surgery_xiandao_merge_20260926`）＝ **删 2**（`company_FirstRareMaterials`／`company_GuangdongFirstRare`）· **改 2**（`company_Xiandao` 仅 aliases＋updated_at；`company_XiandaoGroup` 五字段）· **增 1 边**（`rel_company_Xiandao_part_of_company_XiandaoGroup`，落于边序末）· **改 3 边**且自写字段级 diff 逐条打印改前后值，**仅 `source`＋`updated_at` 变动、边 id 留旧、其余字段全等**（3 条 id 与墓碑 `edges_retargeted` 逐条对应）。③ **A3 成立**：两件墓碑 4 个 `absorbed_full_nodes` 整节点（11／11／12／14 键）与**对应术前备份逐字段全等且键集全等** ⇒ 节点层可单独复原；墓碑 id 集＝图中实际被删 id 集（两侧相等）；两 `backup` 字段所指文件实存。④ **A4 成立**：canonical **6449／7118**；自算重复节点 id／悬挂边／自环／边 id 重复／同三元组／空 type（**节点与边两侧**）／缺 id **全 0**；`metadata.total_nodes/total_edges` 与数组实测逐数相等；**13** 条 `_meta_surgery_*` 的 backup／tombstone 路径**逐条实存**。⑤ **A5 成立（其中「原名」一项以重建证成）**：原名卡已消、`wiki/_DEPRECATED_firstrarematerials_20260926.md` 在场（913 B，`图谱ID=company_FirstRareMaterials`，`mtime` 08-22 01:46:13 而 `ctime` 09-26 07:01:58 ⇒ **内容当日未被重写、仅元数据变动**）；该 id **canonical 无／P2 有**；幸存方卡 `wiki/xiandaogroup.md` 在场（`ctime=mtime=08-22`，未被触碰）。**原名证法**：本审核者以 `wiki_autogen.render_card()` 对 **P2 图**该节点重跑，产物与盘上弃用卡**仅差页脚生成日期一行**（08-22 vs 今）⇒ 该卡确由 `wiki_autogen.py` 生成，而该脚本落盘名恒为 `slug(id).md`＝`firstrarematerials.md`；另全库 **39 张带 `图谱ID` 的 `_DEPRECATED_*` 卡 39/39 满足 stem＝slug(图谱ID)**。⑥ **A6 成立（源码＋实跑双证）**：`wiki_autogen.py` 仅 `dest = WIKI_DIR / f"{slug(nid)}.md"` ＋ `if dest.exists() and not force: skip`，全文无删除／改名 stale 分支；把 `WIKI_DIR` 重定向到 `/tmp` 后预置一张同名 stale 卡跑全量 regen —— **新建 981 张、该 stale 卡原样留存（字节全同）**，真 `wiki/` 2302 张零变化 ⇒ **不自愈**实证。⑦ **A7／A8 成立（口径＝紧邻格式归正之前的 P3 态）**：P3 全库 **props 值含 `**` 的节点恰 1**＝`company_Xiazhi.high_ar_drill_monthly_output_2026_07_highAR`（含嵌套／非字符串值扫描＝0 越界），同态 `description` 含 `**` 者 **48** 个（远多于 1，合法惯例）；P3 **desc 结尾带半角空格者恰 1**＝`company_XiandaoGroup`。**补口径**：上溯至厦芝术前的 P1，此两项**均为 0** ⇒ 两项确系 06:39／06:41 两件引入，与本件根因声明一致。⑧ **A9 成立**：P3↔canonical 逐 dict diff ＝ **恰 2 个节点字段**（`company_Xiazhi.properties`／`company_XiandaoGroup.description`）· 增删节点 0 · **边零改动** · 计数未变；值内 `**` 去壳后**长度 47→43**（恰少两对星号；去壳后字符串全等 ⇒ **文字一字未改**），desc **527→526** 且 `desc_new == desc_old.rstrip()`；`company_Xiazhi` 的 **props 键集（14 键）与 `region`＝「中」均未动**。⑨ **A10 成立**：`outputs/verify_xiazhi_xiandao_replay_20260926.py` 本审核者实跑 **exit 0 · 硬检查 30/30 全过**；**只读性两重实证**——跑前后 canonical `sha256=6bb3cd6d…` 与 `_health.json` sha256 不变，且**全仓 7477 个文件的 mtime／size 指纹跑前跑后逐行相同**（其 4 件 QA 子进程的写只落 `/tmp/myg18_*`）。**防篡改旁证**：P1 的 `sha256=bb93652d…` 与更早一轮审核者当时记于本文件的 canonical `bb93652d…` **同值** ⇒ 三件备份非事后编造物（另核：`lasttwo` 的 062619／062657 两备份逐字节相同，与本条早前陈述吻合）。⑩ **A11 成立**：`_health.json` `updated_at=2026-09-26T07:04:53-07:00` ≡ canonical `mtime` 07:04:53.299（该字段由 `scripts/write_health.py` 从 mtime 构造、逐秒截断，故属构造性一致而非巧合）；`overall=ok`／`fails`／`warns` 空；counts `nodes=6449`／`edges=7118` 与实测一致。⑪ **A12 成立**：canonical「props 值含 `**`」＝**0**、「desc 结尾带空格」＝**0**，而 `desc` 含 `**` 仍 **48** 个（合法口径未误伤）。⑫ **断言外追加自算**：**别名卫生本场净改善**——全库「同一别名被多节点持有」190→189→**187**，**零新增**，`厦芝`／`先导`／`先导稀材` 由多主归一为主（先导系唯 `company_XiandaoGroup` 持 `先导`）；**下游扫描**：`wiki_autogen.py` 仅以 slug 匹配文件名、不比对字段值；`consumers/龙鱼五力/five_forces_engine_v3.py` 对 props 值／desc 只做小写**子串**检索（删 `**` 只会增配不会减配）；`check_ds_evidence.py` 用中文／数字短语共现；`rules/check_desc_shrink.py` 判阈「缩 >10% 且 >30 字」远大于 1 字；全仓 grep 该旧值字面 **仅见于 backups 与该修复脚本自身** ⇒ **未发现对这两处改动的字面／精确匹配消费端**。**边界（不构成 FAIL · 供后手）**：(a) **A5 的「原名恰为 `firstrarematerials.md`」无保留的改前目录清单可直接观测**（无 `.bak`；本审核者按约束未用 git）——现以「逐字节重建＋39/39 命名约定＋mtime/ctime 分离」三项**合证**，属强推断而非直接观测，标为**未完全核**。(b) **文档措辞过宽（须订正，本审核者未擅改正文）**：`outputs/cc_fix_firstrare_wiki_deprecate_20260926.py` docstring 与本块 B① 句「38 张已改名 `_DEPRECATED_*_20260926.md`」——实测该 38 张按日期后缀为 **`_20260910`×28 ／ `_20260926`×8 ／ `_20260823`×1 ／ `_20260828`×1**，通配 `*_20260926.md` **只对其中 8 张成立**；其事实核心（39 悬空／38 已弃用／1 活跃、`wiki_autogen` 不自愈）经全量重扫**逐数成立**，故定性同本条 ⑤ 例＝**文档措辞失准、非数据错误**。(c) **派生层未刷**：`wiki/xiazhi.md`（生成 09-24）／`wiki/xiandaogroup.md`（生成 08-22）内容仍为合并前版本，去壳后新值**未出现在任何 wiki 卡**——系 `wiki_autogen` 惯常 skip 行为（本场未引入），且验收口径「活跃悬空＝0」不覆盖「卡内容陈旧」；如需一致须 `--force` 重生成（归 Doctor 裁）。(d) **复验脚本打印层折叠**：`verify_…replay_20260926.py` §1 把**新增边**并入「改边」行打印，故显示「改字段＝[id, source, target, type, description, data_sources, updated_at]」——**仅显示折叠，其硬检查该项判为「增边」正确**，不影响判定。(e) **可重放前提在同机备份**：本审核者实读 `.gitignore` 确认 `backups/`／`outputs/` 均在忽略内 ⇒ 异地复现须先取三件备份；B 段该登记**维持**。(f) **本审核者未核**：`surgery_*` 两脚本的内部实现与干跑覆盖（只核其产物变更面、墓碑与备份三态）、`mapping/` 下历史旁支（`_badrun_*`／`_quarantine_*`／`_proposed_*` 等）、以及 `desc` 内 `**` 在其他渲染端的表现（本场该数 48 既未增也未减）。

**口径补注 2026-09-26（接手场复核 · 承接手场对上一落签段的自算）**：上段落签 ⑫ 报「同一别名被多节点持有 **190→189→187**（中文 61）」，**未声明口径**。接手场以「token 原样去空白 → 持该 token 的节点 id 集合 > 1」在同一四态（P1／P2／P3／canonical）重算，得 **250 → 249 → 247 → 247（中文 117→116→114→114）**——**逐段差值与 190→189→187 完全一致（−1／−2／0），仅绝对水位整体高 60**，差源＝对方口径中某项未声明的收窄过滤。⇒ 落签 ⑫ 的结论（**本场净改善、零新增**）在两种口径下**同向成立**，由接手场独立复算确认；但**绝对数不可脱离口径引用**（正是本条追记 ③ 与 `G-X197` 要防的失准）。
**同段另注**：其边界 (b) 报弃用卡后缀分布 `_20260910`×28 ／ `_20260926`×**8** ／ `_20260823`×1 ／ `_20260828`×1（合 38）；接手场实算为 ×28 ／ ×**9** ／ ×1 ／ ×1（合 39）——**差 1 张同在 `_20260926` 桶**，差源未明（可能对方把本件新增那 1 张从该桶剔除）。**两数并存，以本场实算 28/9/1/1 为接手场口径**；B① 句与本件脚本 docstring 已就地订正。
...
**触发**: 2026-09-11 审计 grep `commodity_prices.jsonl`——L595 `as_of="2028年"`（玻璃基 Interposer 报价·帕米尔 07-08·horizon=预测）、L815 `as_of="20280101"`（2.4T 轻相干批量单价·horizon=预测）；`latest.json` 快照 grep「2028年」命中 2 处。

**根因**: kg_ingest 价格层入库无 as_of 日期格式校验（第 14/15 项门禁查 schema/三元组，不查日期格式）；预测点本身合法（horizon=预测），但 as_of 应为合规日期字符串，LLM 生成的「2028年」直接入库。

**影响面**: 快照层数据质量；白泽按 price-coupling-v1 过滤暂不受影响——但契约只防「预测混入」不防「格式畸形」，直接读快照的消费者会拿畸形日期做排序/时点判断。

**修法候选（待 Doctor 裁）**: ① kg_ingest 价格层入库加日期格式校验门禁（regex `\d{4}-\d{2}-\d{2}`，畸形即拒/回 LLM 改）；② 存量 2 行清洗（「2028年」→「2028-01-01」或按 note 语义修正）。
...
**触发**: `yuantu_scoring.py` L29-44——所有边无向化（adj 双向 add），以到 TAU_ANCHORS 的图距离与节点度参与评分；`_GRAPH_CACHE` 只加载一次（L31），图谱版本更新后缓存不失效。评分经 KGClient.company_score 进入龙鱼数据包（five_forces_engine_v3.py:1309）。

**根因**: 图距离/连接数能发现关联与覆盖度，不能证明受益强弱；不按关系方向、正负经济含义或边的证据等级区分。新增研究链接可能改变分数，即使企业现金流前景无变化。本轮未回放证明六维总分或操作建议因此改变（VV 边界声明）。

**建议修法（候选）**: 保留相关度/覆盖分；另给经济传导路径（受益方向/收入利润暴露/客户验证/替代路线/兑现时间），只让有经济含义的有向路径影响受益判断；验证用例「新增同义词/引用但无经济新事实时，经济排名不应跳变」；常驻进程随图谱版本失效缓存。
