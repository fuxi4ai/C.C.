...
> 由 `brain/.tools/scheduler_snapshot.py` 生成于 2026-08-17T23:33:39-07:00，**只读**。

> **本文件纳入 git；跑完 `git diff` 即知自上次快照以来什么变了** —— 无论改动来自 Doctor、别的会话还是 CC 自己。

> 镜像步：镜像已同步：21 文件（更新 16 · 移除 12） · Artifacts：7 个（拷 0 · 清单全量）


## 面① Cowork live 树（21 个）
...
| `event-attribution-watch` | 2026-08-15 23:13 | 50 | `f1c24629964c` | 事件归因验证班：采集上一美股交易日数据→事件日判定→收盘级归因→与注册量级带对准→提案制汇报（剑酒青丘·解释层） |
...
| `longyu-weekly-dualscorer` | 2026-08-12 22:37 | 63 | `79e76877ba7c` | 龙鱼标的库周更：常更标的每周双scorer打分(deepseek子项+claude top-down)落库+对比校正队列+刷新个股库看板artifact |
...
| `recap-kejian-daily-ingest` | 2026-08-12 18:56 | 30 | `4da3a77db942` | 由九儿扫小鲍课件→四维入recap.db：dim1/dim2全自动、dim3情绪叙述纳入、dim4仓位半纳入(拿不准留待人工复核)，严格去重标P2 |
| `recap-kejian-review` | 2026-08-12 18:57 | 33 | `ce147c5c1b29` | 每天09:30由句芒审核九儿课件入库：去重/数据合理性/P2标签/归位，扩审dim3(禁行情倒灌)与dim4仓位(归一/词表/待复核不重不漏)，出审核日志 |
...
| `scheduler-weekly-audit` | 2026-08-11 09:02 | 71 | `39dc0440467d` | 定时任务四执行面周巡检（只读）：跑 scheduler_snapshot.py，exit 0 则完全静默不打扰；exit 1 才把 🔴 异常清单报给 Doctor。绝不自动修、不 commit、不碰调度器。沙箱无法执行时… |
...
| `zhuzhao-market-fetch-daily-report` | 2026-08-17 11:46 | 97 | `a8ca6263e4e7` | 每交易日由九儿增量拉烛照九阴四表行情(theme_etf/us_anchor/market_amount/limit_list，A股三表走tushare、美股锚走yahoo chart API默认源·盘中守卫+null … |
