...
- **G-09 钨价 SMM 登录锁 + SVG 坐标幻觉（2026-08-31 实撞 · 🔄 已修待验）**：`commodity_prices_live.json` 钨条目出现 price=**1333 元/标吨**（yoy 算出 -99.4%）。**根因（2026-08-31 重抓证伪后定案）**：SMM h5 钨页（hq.smm.cn/h5/tungsten-ore-price）已将黑钨精矿价格列全部上登录锁（价格单元格=登录锁图、login 链接），公开页无价格数字；pandas `read_html` 解析锁定表格时把页面 SVG 图标的 `transform="translate(-1333,-1165)"` **坐标值当成了均价**——row_match 没抓错行，是无数据+解析器幻觉。**放大器**：weekly_health 只查 asof 新鲜度不查量级 → 幻觉价带「age 0 新鲜」进周报/弹性（-99.4% yoy 污染钨弹性排序），健康层不暴露。**处置（已做）**：Doctor 裁「重抓」→ SMM 免费页已死重抓证伪 → 钨条目降级待核验（price/yoy 置 null·confidence 0·source 注记登录锁+SVG 幻觉·备份 .bak_20260901_pre_w_quarantine）→ warehouse 重建+看板刷新。**残留风险**：fetch_smm_h5 对其他品种（若 SMM 同样上锁）会重复产出幻觉价——**预防门禁候选**：解析器加量级守卫（品种合理区间校验，偏离历史均值超阈值即拒）或「单元格为登录锁图时整行判无数据」；修法归 Doctor 裁。**候选源**：中钨在线（免费可爬·信源图谱备选）/SMM 登录态重抓。
