...
> 项目内实时日志已随 monorepo 并入 `Database/行业研究/consumers/龙鱼五力/`（原 `Projects/龙鱼五力/GOTCHAS.md` 路径 2026-08-20 核已不存在）；本文件即权威沉淀+索引。
...
**状态**: ⚠️ 已知风险（2026-08-20 错题本消化 Doctor 裁——引擎口径修正无排期 · 人工 overlay 兜底常态有效〔07-29 江波龙案实践〕· 判别信号明确；若日后改引擎财务维「盈利能力」子项〔近 N 季报表 GM + 穿越周期 ROE〕另立专项）
**优先级**: 🟡 中（周期股/价差型标的必踩）
...
**状态**: ✅ 已知设计（长期维护·2026-08-20 复盘回写——原「长期维护条」状态词不在 conch 计数闭集被误计积压）
**优先级**: 🟢 低
...
**状态**: ⚠️ 已知风险（沙箱专属 · 2026-08-20 错题本消化定——实读引擎 L59 `FF_REPORT_DIR` env 兜底已在〔workaround 可用〕，Mac 原生默认路径正确；改默认值反破坏原生行为，引擎不动）
**优先级**: 🟡 中
...
**状态**: 🔄 待修复（2026-08-20 错题本消化拆两子项：① 升格通用教训——**已落盘** `permanent/通用教训.md` G-X153〔Doctor 裁起草·「龙鱼批」落〕；② 检测机制 TODO 已补挂〔brain/TODO.md「ds 腿引用数字检测机制」条〕· 待闭环）
**优先级**: 🟡 中
**触发**: 2026-08-19 校正队列销账场对海光 688041 复核：ds 腿供需端 16 分依据「中芯产能排期满至 2027 年」——canonical 全图检索零命中，且中芯为华为链，海光 x86 代工为三星（concept_SamsungFoundryCPUConstraint2026 在案），属错链引用。同族第一例：08-15 华工 ds 腿把 P3「2万样机」当事实（Doctor 裁校正 63.5）。
**根因**: ds 腿自下而上判分时引用训练记忆/外部印象中的具体数字，未对 canonical stage/P 级做存在性校验；检测机制尚未实装（08-15 渊图场已挂 TODO）。
**影响面**: 供需端/技术端等子项分可能被未证实或错链数字抬/压；本例方向为抬分（修掉后海光 Δ 反而变宽），单点未扭曲总分结论（销账裁定成立），但属系统性可靠性风险。
...
**⇒ 2026-08-30 第三次复发（实读实据）**: 08-29 周更班海光 688041 ds 腿供需端 evidence 再现「中芯代工产能排满至 2027 年显示供给约束下的强需求兑现」——同标的、同句。**⇒ 2026-08-30 复核推翻「错链」定性（同场实读 canonical）**：该引用在 canonical **有锚**——`rel_Haiguang_SMIC_South`（company_Haiguang → company_SMIC_South · requires · 帕米尔 2026-04-29 · P1）desc 原文「海光CPU产能依赖中芯国际代工，产能排期已满至2027年，供应紧张」与 ds 引用逐字吻合。故 ds 非凭空编造，是引用了 canonical 内**未退役的旧锚**；同主题更新叙事已在库（concept_SamsungFoundryCPUConstraint2026「海光CPU 2026年三星代工产能受限·从台积电转三星·良率不佳」· 05-16 建 · 07-21/08-16 追源）但旧边未标时效/未退役——canonical 内部时序冲突是根因之一。08-19 本条目「canonical 全图检索零命中、中芯为华为链」的裁定基础需复核（结论「海光双分共存销账」不受影响，归因「错链锚为抬分」不成立）。同族第一例（08-15 华工「2万样机」）是否同型旧锚待查。**同族第二次实质复发→应升格——已升格 G-X163（2026-08-31 /save 场 Doctor 勾选落盘 · permanent/通用教训.md）**，08-19 裁定复核与 canonical 旧边处置仍归 Doctor 裁。
**来源**: logs/2026-08-15-渊图15篇入库与光通信龙鱼重打分.md（第一例）+ 2026-08-19 专项校正场（第二例·销账留痕）+ 2026-08-29 周更班 records 实读（第三次）
...
## [NOTE-20260830-001] ds 供需端维度正交污染 + 产能受限方向判反（08-29 周更班 4 只边际分歧簇诊断）
**状态**: ✅ **已验收（2026-09-04 Doctor 落签）**——2026-08-30 裁定场 Doctor 三题全批推荐 → 实施完成：ANCHORS 四补 + 全 23 只重跑覆写 08-29 同日条目 + 检测机制实装 → 2026-09-03 独立验收出报告：PASS_WITH_LIMITS（未参与实施的 subagent 实跑实读 · 核心声称全部验证通过 · 两条限制不阻断：⚠67→70 系 canonical 图谱 09-01 入库后的环境漂移·光库 v3 技术端毛利率串维残留属范围外观察）→ 2026-09-04 Doctor「落签」批准 ✅（CC 代笔留痕 · 同场销 TODO 条）
**优先级**: 🟡 中
**触发**: 2026-08-29 周更班 4 只边际分歧簇（仕佳 Δ-14/源杰 Δ-14/光库 Δ-13/海光 Δ-11 · 全同期 0 天 · 最大分歧维均=技术供需）子项级诊断——三只供需端 evidence 含锚点违反，一只干净。
**硬证据**（records 08-29 ds 条目实读）:
- 仕佳 688313：技术端已扣「EML 良率 40%+ 低于海外 60-65%、CW 仅样品验证」，供需端再以「EML/CW 尚未批量出货·新品类兑现度不足」压至 14（部分兑现带）——同事实双维扣分，违反「负面只在一维扣」（维度正交铁律）。
- 光库 300620：供需端以「OCS 代工毛利率低、高端 FAU 产能受限」压至 14——①毛利率属议价/盈利能力指标，串维扣供需；②「产能受限」按 Doctor 07-06 制度（制度 §兑现折扣例外 + ANCHORS v2 白纸黑字「供给约束的低增速=需求强兑现→给高供需 16-23 不打折扣」）是需求强证据，被**判反方向扣分**。
- 海光 688041：供需端混入「DCU 算力落后寒武纪 690」——「落后」属技术/竞争判据；同条 evidence 再现中芯错链锚（NOTE-20260819-001 第三次复发）。
- 源杰 688498：供需端 evidence 干净（17 分·理由全为公司自身兑现证据），分歧主因在技术端尺度（「与头部差距显著」）——**不属本族**，诚实标注。
**根因**: ANCHORS 供需端锚点对「新品类进度归位」「盈利能力指标归位」「产能受限方向」无显式禁止/钉死条款；ds 未严格执行既有条文（07-06 制度）。供需端 23 分为最大权重子项，污染系统性压低「高兑现+新品类待放量」标的。
**影响面**: 本周 23 只常更至少 3 只 ds 总分被污染压低；光通信组 ds 运营Δ连续两周下行（08-25 观察条：华工 -11/盛合 -7/旭创 -6/新易盛 -6）疑似同源（未证实·下周 ds 腿复看）；双腿 Δ 系统性偏向「claude 高估」假象。
**建议修法**: ANCHORS 供需端锚点四补——①兑现=公司级营收/订单/份额落地即算；②新品类进度（验证/良率/批量出货）只归技术端/新赛道，供需端禁二次引用；③盈利能力指标（毛利率等）逐出供需端判据；④产能受限方向再钉（供给约束=需求强→16-23）。
