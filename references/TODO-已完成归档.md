...
- [x] **渊图 · `product_NvidiaGB300Rack` wiki 卡缺位（2026-09-26 /todo 自「wiki 派生层陈旧」条拆分 · 源：`logs/2026-09-19-大摩VR200机柜BOM核实与回填.md`）**：canonical 有节点 `product_NvidiaGB300Rack`（实读 id 在册），但 `wiki/product_nvidiagb300rack.md` **不存在**。成因＝`wiki_autogen.py` 默认只建**缺失**卡、且**只对度 ≥ `min_degree`（默认 4）的节点生效**；两次 BOM promote 走的又是 `kg_merge_safe`（**无 wiki 步**）。**（2026-09-26 客观代勾 · G-X136 常驻授权 · 证据：`python3 wiki_autogen.py product_HBM4ForVR200 product_NvidiaGB300Rack` → exit 0 · `完成：新建/更新 2，跳过 0` · wiki 卡数 2300→**2302** · 现有 2300 张 mtime 仍 09-24 09:56 零触碰 · 新卡 `product_nvidiagb300rack.md` 1133B 与 `product_hbm4forvr200.md` 1552B 均在盘且正文含新口径段）**
  **⚠ 修法订正（同场实测）**：本条原记「修法＝跑一次 `--force` 全量刷新」——**该修法对这两张卡无效**。两张卡的度**均为 1**，低于 autogen 的 `min_degree=4` 阈值；而 `--force` 分支的筛选式是 `[nid for nid, deg in degree.most_common() if deg >= min_degree]`，**同样过该阈值** ⇒ 全量 `--force` 只会白白重写现有 **2300** 张卡、**建不出**这两张。**正确做法＝显式指定 node id**（`args` 分支绕过度阈值过滤）。
...
- [x] **DVA · GOTCHAS 双源分叉待裁（2026-09-17 /save 挂）**：brain 索引与 08-06 纪律称权威＝`Projects/DVA/GOTCHAS.md`（最新 ERR-20260911-001 · 09-16 改），但 Codex 交接与本场三条 ERR-20260917-004/005/006 均写 `Codex/Project Mirror/DVA/GOTCHAS.md`（09-1x 已 65 条，Codex 日常在写）——纪律与现实相反，归 Doctor 裁：权威改镜像仓 / 回流合并进 Mac 库 / 双轨各管各。（✅ Doctor 2026-09-17 /todo 裁「权威改 Codex 镜像仓」· CC 代记并执行：Mac 库 GOTCHAS 头部加指针注记转历史副本 + brain/DVA/GOTCHAS.md 纪律反转注记 + 回流方向反转〔Codex→Mac 汇报制〕）

- [x] **龙鱼 · 09-05 周六周更班 v3 锚点继承首验（2026-09-01 /todo 漏挂对账补挂 · 源：`logs/2026-08-31-龙鱼ds供需四补v3与检测机制.md` · dated 09-05）**：v3 四补 23 只重跑后首轮周更——验锚点继承（攒满 2 个 v3 点后趋势自动恢复可信）。（Doctor 2026-09-17 /todo 统一授权勾销 · CC 代记——证据：趋势文件 09-03/05/10/12 连续在盘〔`Database/龙鱼-标的分析库/` 分数趋势件〕· 09-12 周更场正常；内容级锚点继承结论未核，以班连续运行为证据销）

- [x] **EAL · 班新 prompt（SHA `2547c761`）首个真实 scheduled run 简报核（2026-09-03 /todo 漏挂补挂 · 源：同上）**：现核班在跑（lastRunAt 09-02 17:47 PT ✓）· 简报内容未核——班后核行情水位/selector 解析/loop 状态/artifact 回读。（Doctor 2026-09-17 /todo 统一授权勾销 · CC 代记——关注已被 NOTE-20260911-001/X1 链取代；09-17 晚班零产物系新问题，另挂待办）

- [x] **DVA · Codex 仓 commit+push（2026-09-17 挂 · 命令块已贴 Doctor 终端）**：25 M + 3 新文件（含 Codex 当日 12 处），排除 graphify-out/ops/pycache；随后处置四个 Fuxi staging 目录、`/private/tmp/dva-*-release/` 与 helper 脚本、Fuxi `%TEMP%\dva-writer-role-test-*`。Codex 侧余项见交接件 §8（旧正则两处 · 157→168 期望 · 去 `.cmd` launcher 验证）。（CC 代勾 2026-09-17 /todo · 证据：Codex 仓 chore/workspace-reconcile 分支 3 commit 已 push·local=origin 对拍一致·工作区 find 无未提交件；fuxi staging 处置子项随交接件 §8 跟踪未核）

- [x] **渊图按需更新组核实批 · P1 完成收尾（2026-09-13 挂 → 09-13 续跑完成 wave3+wave4 · 源：`raw/核实/2026-09-13-渊图按需更新组核实札记.md`）**：✅ ①wave3 重跑完成（24 篇/151 断言/17 订正落盘·上一轮 3 遗留：猛禽+OpenRouter 落盘、V4.1 时间线裁决坐实）；✅ ②wave4 完成（44 篇/36 订正落盘/2 防诈骗帖跳过）→ **新 116 篇全量核实完成，订正累计 88 处（全经独立 QC+sha256 ledger）**。③ ~~P2 阶段复用 210 篇去重口径~~ ✅ 2026-09-13 完成（组 `_evidence/2026-09-13/p2-dedup-reconciliation.json`：A 09-09 批 16 已入库已抽查 / B 08-16 批入 canonical 126 普查覆盖 / C 08-16 批视角层 67 未断言核实〔30 产业金融类/37 地域杂类〕/ 派 Scale-in 1 已双口径入库；**C 档补核已裁已做**：Doctor 裁「只补产业金融 ~27 篇」→ 精筛 30 篇 → 5 核实员+QC → 27 订正落盘〔ledger `cc-verify-corrections-cwave.json`·核实批累计 141 处〕；C 档剩 37 篇地域杂类维持视角层）；④ ~~入库候选清单三桶~~（✅ 2026-09-13 Doctor 裁「now，批准」→ 已出：`raw/核实/2026-09-13-渊图按需更新组入库候选三桶清单.md`）；⑤ ~~wave2 3 条 unchecked_quota 资产遗失并入 P2 重扫兜底~~（✅ 2026-09-13 去重后确认 A/B 档断言面已覆盖其所属篇目，不需单独重扫）；⑥ ~~未核断言补跑~~（✅ 2026-09-13 三轮收口·Doctor 令+「会话都存了，可以找」：对账 126 条〔日志 124 系 wave4 少记 2〕→ 12 核实员+3 QC → 未核桶 21 篇全量重核+41 残留定向+坐实/存疑桶 52 条 29 篇重抽 → 新订正 39 条落盘〔P1 新 116 篇累计 127 处；含 C 档全口径 154〕→ 剩 3 条不可核/无信源；ledger `cc-verify-corrections-supplement{,2,3}.json`；三桶清单补跑节三轮已落）；⑦ ~~存疑专项~~（✅ 2026-09-13 Doctor 裁「能核出对错的——修改；核不出的不进图谱」：31 核实单元→4 核实员+QC〔confirm 29/adjust 5/reject 0〕→31 订正落盘〔ledger `cc-verify-corrections-cdoubt.json`〕+不采清单 4 条落三桶清单第七节+争议点表 3 行更新〔TPP/长鑫DDR4/猎鹰9〕；全口径订正累计 127+27+31=185 处）；⑧ ~~坐实桶 95 篇入图~~（✅ 2026-09-13 Doctor 裁「坐实桶进图谱」：47 精确+48 批次→3 路 extract→QA 修补〔悬挂16/自环2/非法type2/大小写2/stub1〕→promote 门第16项拦 48 同名实体→并入存量〔墓碑 48 条〕→Doctor 终端 promote 全绿→**canonical 5583/6220 → 6259/6893**·wiki 954 卡·备份在盘·中间件已清·**git 已提交推送 ✓（行业研究仓 8505d3d · brain 仓 efd6d34 · local=remote 实读对拍）**）。执行资产：本会话 `outputs/verify-20260913/`（assignments）+ ledgers 在组 `_evidence/2026-09-13/`（wave1-4 + p2-dedup + cwave + cdoubt）。（CC 代勾 2026-09-17 /todo · 证据：canonical 主库 mapping/行业知识图谱_完整数据库.json 18.6MB 在盘·行业研究仓 local=origin=78c152a9 已推·`"desc": ""` 计数 0）

- [x] **剑酒青丘 · 数据仓 877 untracked 耐久化（2026-09-03 /todo 漏挂补挂 · 源：`logs/2026-09-01-DVA盲审对账审计与EAL验收核验.md`）**：含 eal_scheduled_consumer_v1 四件验收资产 · 无 upstream 本地-only 仓——commit 或另裁处置。**⇒ ✅ 2026-09-04 Doctor 裁「全量建基线 commit」（AskUserQuestion 推荐项）→ 已执行（Doctor 终端）**：commit `60afe1c0`（验收资产四件+宿主侧补跑产物）+ `32474cd2`（全量建基线 877 untracked 一次入库 · 09-04 09:29 PDT）· .git/logs/HEAD 实读 · refs/heads/master=32474cd2 对齐 · 本仓无 upstream 不 push · CC 代勾留痕（2026-09-05 /save 完成即迁 · CC 搬运留痕）

- [x] **渊图 · bak_20260820_diaoyan 归档（2026-08-20 挂 · dated 08-27）**：调研情报局入库场回滚点（4962/5544）——conch 归档批 P1 延后 7 天（.conchconfig.yml protect 已挡）；08-27 后摘 protect 并归档 archived/。（2026-08-27 到期提醒班现核：文件仍在 mapping/（12,672,205 B · mtime 08-19 08:43）、protect 第 9 行仍含该条，均未处置；归档命令已贴 Doctor 终端待执行，未跑任何 git/conch 命令）**⇒ ✅ 2026-09-04 晚场现核销账（CC 代勾·A 类证据）**：09-03 08:40 commit `9458cb64` 实读含「diaoyan回滚点归档」· mapping/ 已无该文件 · archived/在盘 · .conchconfig.yml 第 9 行已改注「已归档 2026-09-03」（mtime 09-03 08:40）· HEAD=origin 同步（2026-09-05 /save 完成即迁 · CC 搬运留痕）

- [x] **渊图 · PTFE 树脂端 A/B 档补丁待裁（2026-09-01 /todo 漏挂对账补挂 · 源：`logs/2026-08-27-渊图-VeraRubin-PTFE材料核实.md`）**：A 档（PTFE 树脂端缺口：东岳/中英科技/Rogers/Chemours/Daikin/巨化/昊华补中昊晨光身份 + 2 概念节点）+ B 档（CCL 涨价 prop/M10 测试口径/IEC 标准/联瑞募投 props）——方向性补丁，待 Doctor 裁。**⇒ 2026-09-03 Doctor 裁「A/B 档全入」→ patch 已备（`mapping/_v3_20260903_PTFE树脂端AB档补丁_manual.json` · dry-run 全绿：8 新节点/5 update/6 边）· promote 命令已贴 Doctor 终端**⇒ ✅ 2026-09-04 晚场现核销账（CC 代勾·A 类证据）**：promote 已于 09-03 08:40 执行并入 commit `9458cb64`（「PTFE树脂端AB档补丁(8新节点/5update/6边·Doctor 09-03裁全入)」）· canonical 实读 5305/5926（= 5297/5920 基准 +8/+6 精确吻合）· 8 新节点/6 新边/5 update id 级全 ✓（updated_at=2026-09-03）· HEAD=origin 已同步（2026-09-05 /save 完成即迁 · CC 搬运留痕）

- [x] **渊图 · 华勤节点「总营收」口径注记（2026-09-01 挂 · 源：`logs/2026-09-01-渊图融资批与盛科51.2T深挖手术.md` 毛利分析场）**：`concept_HuaqinSupernodeRevenueTarget` 的 total_revenue_target_2026e（72）/2027e（90-98）系**数据中心板块口径**（2025 该板块实为 754.75 亿 · 公司总营收 1714 亿）——props 加口径注记防下游误读。小修 · 下次 /todo 或顺手修。**⇒ 2026-09-03 /todo 统一授权执行：patch `mapping/_v3_20260903_华勤口径注记_manual.json` 已备（dry-run 绿·1 update took_patch）· promote 命令已贴 Doctor 终端**⇒ ✅ 2026-09-04 晚场现核销账（CC 代勾·A 类证据）**：promote 已执行并入 commit `9458cb64`（「+华勤口径注记」）· canonical 实读 desc 含『数据中心板块口径』+ props.total_revenue_scope_note 在盘 · updated_at=2026-09-03 · HEAD=origin 已同步（2026-09-05 /save 完成即迁 · CC 搬运留痕）

- [x] **龙鱼 · GOTCHAS NOTE-20260830-001 独立验收（2026-09-03 /todo 漏挂补挂 · 源：`logs/2026-08-31-龙鱼ds供需四补v3与检测机制.md`）**：~~🔄 已修待验（ANCHORS 四补+23 只重跑+检测机制实装 · 实施者不自标 ✅）——待未参与实施的 subagent 独立验收或 Doctor 指定验收方。~~ **✅ 2026-09-04 闭环销账**：09-03 独立验收 PASS_WITH_LIMITS 报告已出 → 09-04 Doctor「落签」→ GOTCHAS 状态行改 ✅（CC 代笔 · Doctor 署名）。（2026-09-05 /save 完成即迁 · CC 搬运留痕）

- [x] **龙鱼 · ds 供需端兑现锚点维度正交漂移订正候选（2026-09-01 /todo 漏挂对账补挂 · 源：`logs/2026-08-29-龙鱼双scorer周更.md`）**：deepseek 供需端兑现疑似维度正交漂移（仕佳年度 +98%/+473% 仍给供需 14「部分兑现」·海光「DCU 落后」混进供需端）——候选修法＝供需端兑现改公司级营收/订单/份额落地即算，新品类进度归技术端/新赛道维；批准则改 ANCHORS+重跑+趋势标口径跳变。归 Doctor 裁。**⇒ 2026-09-03 Doctor 裁「ds 供需订正」（修法批准）→ 实施=改 ANCHORS+重跑 23 只+趋势标口径跳变 · 大活待开专场**。**⇒ ✅ 2026-09-03 专场实施完成 + 2026-09-04 Doctor「PRD验收通过」总签 → PRD delivered（`logs/checkpoints/2026-09-03_龙鱼ds供需订正_PRD.md` · R1-R5/N1/N2/X1 全部 [✓] · R3 光库 evidence 残留裁「接受现状不打回」· 残留待裁两件仍在 PRD open_decisions：检测第四档串维短语/挂周更班）· CC 转录销账**（2026-09-05 /save 完成即迁 · CC 搬运留痕）
...
- [x] **渊图 · 胜宏科技 M9/M10 认证法定披露校验窗口（2026-08-05 挂 · 2026 年 8 月底半年报）**：核实札记已定级——「M9 材料产品验证完成」P2 可坐实，「全球唯一通过英伟达 Rubin 正交背板验证」坐不实（公司 03-18 法定口径「正积极推进」、07-31 董秘回避）。**8 月底半年报是第一个法定披露校验点**：届时核对 M9/M10 认证表述是否升级，据此决定 hedged patch 起草/作废。旁线：SemiAnalysis「Rubin 因正交背板卡点延迟一年量产」（P1 外媒，与「已过验证」叙事正面冲突）若被英伟达官方/供应链否认 → 反向信号降级；若被证实 → 本身即图谱级信号（正交背板卡点 → VR200 子图），可另立核实。**⇒ 2026-08-19 新增对照素材**：帕米尔 08-18 PCB 调研口径「胜宏 Vera Rubin 批量交付 & 高多层份额提升」（调研口径 P2·非法定）已入库——半年报校验时与此调研口径对照，若半年报仍无 Rubin 认证升级表述，则「批量交付」维持调研口径不升格。**⇒ 2026-08-25 新增对照素材**：SemiAnalysis 7 月爆料「Kyber NVL144 正交背板卡点延迟 12 个月+ → 2028」部分证实且范围收窄——延迟仅 Ultra 相，2026 主代 Vera Rubin 5 月底已规模量产（黄仁勋 7/10 澄清 2027 轨道不变·Plan B 降层数/回铜缆）；对胜宏 M9 材料的双重含义：认证需求推迟一年 + Plan B 材料降级风险。详见 `raw/核实/2026-08-25-VeraRubin实测与AI服务盈利核实札记.md`。**⇒ 2026-08-25 校验场预核（CC 实查）**：披露日官方钉死 **2026-08-28**（互动易 07-31 答复 + 08-07 异动公告双源 P0）；期间无可靠新料——08-24 韭研公社聚合帖「M9 获 N 认证·仅胜宏小批量出货」定级 P3 不入证据链（来源未知+作者持仓+主语混乱+借名「天风郭明錤」发 M10 测试消息）。**核验靶点清单（08-28 披露后逐项核）**：①半年报 M9/M10 认证表述 vs 03-18 基线（「M9 推进中」）——升级一档即可坐实帕米尔「批量交付」调研口径，仍无表述则维持调研口径不升格；②图内 `company_Shenghong`（Rubin Compute Tray 一供 75% 调研口径 props）；③图内 `company_VictoryGiant` desc「正交背板独家供应商」强断言与景旺送样节点（`concept_RubinUltraOrthogonalBackplane` 景旺 78/84/108 层送样）图内矛盾；④`concept_PTFETestProgress2026Q3`「7-8 月确定最终方案」dated 断言**已顺延**（8 月口径 Q4 或之后·英伟达未官宣 BOM 锁定仅立验证 spec）——须与半年报 PTFE/M10 表述分开核（M9 碳氢+石英布=量产主力 vs PTFE 无布=M10 级候选）；⑤**双胜宏节点并存**（VictoryGiant 06-19 建 / Shenghong 08-18 建·aliases 互指——同名双 id 族系）手术待裁；⑥VictoryGiant region=港 疑港股代码 02476 误配（A 股 300476 应=中）；**⇒ 2026-08-27 靶点增补（CC 实读新挖·C 档·札记 §6）**：⑦**双节点并存族**（超出靶点⑤双胜宏·同族手术待裁，勿与 08-28 核验混批）——双生益（`company_Shengyikeji`/`company_ShengyiTechnology`）·双菲利华（`company_Feilihua`/`company_PhilipsGlass`）·双日东纺（`company_Nittobo`/`company_NittoboGlassCloth`）·双宏和（`company_Honghe`/`company_HongheElectronicCloth`）·双斗山（`company_Doosan`/`company_DoushanCCL`）·双泰山（`company_TaishanGlass`/`company_TaishanGlassFiber`）·双巨石（`company_ChinaJushi`/`company_JushiGroup`）·三光远（`company_Guangyuan`/`company_HenanGuangyuan`/`company_LinzhouGuangyuan`——光远新材位于林州·疑同实体三节点）·三 Q 布（`material_QGlass`/`material_Q_Cloth`/`material_QClothQuartz`）·双 T 布（`material_TGlass`/`material_T_Cloth`）；⑧边 id 与端点不一致 2 条：`rel_Jingwang_VeraRubinMidplanePCB`、`rel_Jingwang_OrthogonalBackplane`（id 带 Jingwang·source=company_Pengding——景旺↔鹏鼎混标族）；⑨语义污染边 `concept_VeraRubinMidplanePCB -is_a-> product_Midplane`（后者是人形机器人中板·高盛机器人报告源）+ 方向疑点边 `concept_RubinSwitchTrayMaterialShare -used_in-> material_M7M8CCL` + `company_ShengyiTechnology` desc 主语漂移。**⇒ 2026-08-26 /todo 裁定并入本场**：烛照 SA 应急通道 QQQ 口径污染根治（QQQ 值写入覆盖 ^NDX 行 · 是否另立 code）由 Doctor 裁「并入 08-28 核验场一起裁」。
**⇒ ✅ 2026-08-28 半年报核验提前完成（Doctor 裁「现在核」· 报告 08-27 盘后已出）**：靶点①M9/M10 认证**维持不升格**（全文 0 命中英伟达/Rubin/M9/M10/PTFE/正交/背板）· ②未提及 · ③「正交背板独家」加口径注记（Doctor 裁）· ④顺延不变 · ⑤双胜宏已并（Doctor 裁「现在并」· VictoryGiant 主节点 · 墓碑在盘）· ⑥原假设修正（02476 系本公司 H 股代码·region 已归中）· ⑦C 档双节点族**剩 9 组**另排专场 · ⑧QQQ 已裁「另立 code」并改完自验（烛照 GOTCHAS ERR-20260828-001）· ⑨财务 hedged 注记已批已入（9 键）。核验札记 `raw/核实/2026-08-28-胜宏半年报核验札记.md` · 待独立验收（实施者不自签）。**⇒ ✅ 2026-08-28 C 档手术同晚闭环（Doctor 令「修复」）**：10 组双/三节点族全部合并（生益/菲利华/日东纺/宏和/斗山/泰山/巨石/光远三合一/Q布三合一/T布）+ 边修复 4 件（鹏鼎→景旺重指 3+机器人中板歧义重指+删 2 污染边）→ canonical 5162/5793 · 墓碑+手术记录+备份在盘 · 待独立验收。本条目全部子项闭环，整条可销；仅剩宏观层面「ERR-20260602-001 族系升格」由 Doctor 裁。
**⇒ 2026-08-30 预核班独立复核（shenghong-h1-verify 定时班实跑 · 任务 enabled=false 本次为 lastRunAt 记录）**：结论全部复现——半年报全文独立通道（证券之星镜像 SN2026082800005739 · 854KB）逐词复检与 08-28 grep 表逐字吻合（认证=4 / 泰国=21 · 英伟达/NVIDIA/Rubin/M9/M10/PTFE/石英/碳氢/正交/背板/新材料/陶瓷/埋铜/Vera/TPU/V8 全 0）；财务面多源一致；靶点⑥独立坐实：02476 系本公司 H 股（2026-04-21 港交所挂牌 · 209.88 港元 · 净额约 198.89 亿港元）→「误配别家」假设证伪。反查新发现（防误引）：网络 AI 摘要所谓半年报 MD&A「推进石英玻璃布、PTFE、陶瓷、埋铜等新材料研发」句**不在正文**（摘要混源）。hedged patch 建议**作废**（不起草认证升级型 patch）。本班零 canonical 改动、零手术、不动 checkbox、不自签——整条销账与 ✅ 落签仍归 Doctor。
  依据：`Database/行业研究/raw/核实/2026-08-05-胜宏科技Rubin验证核实札记.md` · `logs/2026-08-05-渊图-胜宏科技过验证P2核实.md` · `raw/核实/2026-08-27-VeraRubin PTFE材料与CCL电子布影响核实札记.md`（§6 C 档）（2026-09-01 /todo 全勾统一授权 · Doctor 勾定 · CC 代记留痕并迁档）

- [x] **渊图 · 存算三孤岛节点补边题材（2026-08-25 由 /todo 漏挂对账补挂 · 源：`logs/2026-08-22-渊图OCS深挖与德科立300台核查修复.md`）**：SamsungHBM_PIM/NeuRRAM/中国存算一体需求红利三孤岛节点补边题材——内容工作，等 Doctor 提起。**⇒ ✅ 2026-08-31 现核销账（Doctor 存算一体提问场 · CC 实跑实读）**：三节点度 4/3/5，均已出孤岛（08-23 Ayar 孤岛补边批已落实 · 概览 08-23 条实证「存算三孤岛出孤岛」）；本条目系 08-25 补挂时照抄 08-22 旧源未核现况，工作内容已不存在。迁移归档待 /todo 统一批。（2026-09-01 /todo 完成即迁 v3.1 · CC 搬运留痕）

- [x] **渊图 · kg_ingest 日期漂移 batch 收尾显式汇总（2026-08-11 目标模式漏挂补挂 · 源：`logs/2026-08-05-渊图3篇入库_CCL年份偏移修补.md`）**：batch 收尾把「解析日期≠文件年份」的偏移逐篇显式汇总成一行（07-28 已建议未实装、08-05 第三次复发）；首验=下次 batch。✅ 2026-08-25 首验通过（6 篇批 · kg_ingest 自打两行：散热材料 08-24 vs 解析 08-20；磷化铟 as_of 双漂移 2026-07-01/12-31）——机制已实装并首次生效，销账留痕。（2026-08-25 /todo 完成即迁 v3.1 · CC 搬运留痕）

- [x] **X-Board · 要点晚到件核验（2026-08-22 挂 · dated 08-23 班后 · ✅ 2026-08-24 核验闭环 · Doctor 裁）**：核验结论——**「晚到件」假设不成立**：16 篇中 15 篇是 05-28~06-23 历史视频 ASR 回填（早于锚且不在看板·「在板 False」实核）、1 篇过锚（7676801·08-22 新视频）；真根因=16 篇转写全为新 shape（顶层 aweme_id/backend/model/language/text·mtime 全 08-22 12:38 同批·backend=local-qwen3），提取器/生成器只读 sentences → 全会提取失败。**修法（2026-08-24 Doctor 裁）**：① 兼容改动已落盘（extract_points.py L176 / gen_xboard_artifact.py L150 各加顶层 text 回退）；② 7676801 当场补提要点（points/7676801507991506164.json 在盘·今晚 18:01 班录入后板上可见）；③ 15 篇历史存量裁「接受回退不补」。剩余：VV 知会数据侧 shape 统一归位（另挂条）。依据：`logs/2026-08-22-XBoard双PRD总签与回补裁定.md` · PRD X1。（2026-08-25 /todo 完成即迁 v3.1 · CC 搬运留痕）

- [x] **龙鱼 · 长鑫/南亚 deepseek 补腿（2026-08-11 由 /todo 漏挂对账补挂 · 2026-08-22 /todo 现核改写 · Doctor 裁 A「今晚班带上」 · ✅ 2026-08-24 闭环）**：08-24 /resume 场核验——根因=「班 prompt 纳入两只」裁定未落盘（班 SKILL.md 实读无 688825/688519·且两只不在常更清单〔23 条实读〕），08-22 班照章未跑。Doctor 裁「现在补跑+班注记」→ CC 当场沙箱实跑补腿落库（records 实读：长鑫 ds 63.5/claude 62.0、南亚 ds 56.0/claude 58.5，双腿 2026-08-24 齐备）；班 prompt 处置=退役 CPO 过期特跑段（Doctor 裁）；两只如需周更=常更清单裁定归 Doctor。（2026-08-25 /todo 完成即迁 v3.1 · CC 搬运留痕）

- [x] **白泽大宗 · 周日班后核查（2026-08-08 专场挂 · ⇒ 2026-08-09 Doctor 目标模式裁：改 dated 08-16 班后）**：weekly_health.py 带新 legacy 登记的**定时链路首验顺延**——08-09 01:09 定时班因三目录未挂载**阻塞未跑新码**（`logs/2026-08-09-白泽周报阻塞-挂载缺失.md`），当日晚 Doctor 补挂后手动补跑成功，但 ERR-20260721-001 族「手动通≠定时通」⇒ 定时首验 = 下周 08-16 01:09 班，班后查产物/stderr；ingest_meta 三行 deprecated（Doctor 终端脚本跑完后）只读复查；_health.json legacy_sources 应含两旧价表。四品种鲜价过期由该班处理（龙鱼 3 件②），与本条互证。**✅ 2026-08-22 /todo 现核勾销（Doctor 问答板勾定 · CC 代记留痕 · 证据：班 lastRunAt 2026-08-16T08:09Z ✓ · `data/weekly/_health.json` generated 08-16T01:23:50Z overall=ok·legacy_sources 含两旧价表 ✓（实读）· artifact baize-weekly-dashboard updatedAt 08-16T08:26Z ✓ · ingest_meta deprecated 现核 5 行在盘〔08-08 批 3 行+旧存量 2 行·原口径「三行」〕· 四品种鲜价 08-11/08-16 均已鲜）。隐患：班 stderr 在 Gateway 侧未核。**

- [x] **龙鱼 · CPO 补分班后核（2026-08-12 挂 · 收窄 2026-08-14）**：数据包已落库（08-13）+ 班 prompt 已加「本轮 CPO 补跑」段。**剩**：08-15 周六 dualscorer 班后核天孚/源杰/仕佳/炬光 4 只 LLM 全分（光库重打与否随班裁）+ 源杰 Δ−19 校正一并了。**✅ 2026-08-22 /todo 现核勾销（Doctor 问答板勾定 · CC 代记留痕 · 证据：班 lastRunAt 2026-08-16T05:01Z ✓ · records 实读四只 08-15/16 周更分在盘——天孚 68.0 / 仕佳 59.5 / 炬光 61.5 / 源杰 67.5，源杰 Δ−19 校正随 08-19 销账场并入）。**

- [x] **龙鱼 · 天孚/仕佳收尾（2026-08-12 挂 · 收窄 2026-08-14）**：入组已做（细分环节.json 在盘含两标的 + records + build_index 118 标的）；看板重建 **✅ 02:21 已跑**（117 标的/持仓 12/artifact 已覆盖，Cowork 重开即新）。**剩**：周六班六维全分（并入 CPO 补分条）。**✅ 2026-08-22 /todo 现核勾销（Doctor 问答板勾定 · CC 代记留痕 · 证据：入组+build_index 118 标的 ✓ · 看板重建 02:21 已跑 ✓ · 六维全分=CPO 补分条同批 records 证据 ✓）。**
...
- [x] **龙鱼 · 待校正队列两条旧账（2026-08-15 畸形收口场 /resume 补挂 · 源：`logs/2026-08-15-渊图15篇入库与光通信龙鱼重打分.md` 遗留）**：奥比 688322（08-15 现核 Δ+10.5 · ds 80.0 vs claude 69.5）、海光 688041（现核 Δ−10.5 · ds 50.5 vs claude 61.0；昨日日志「+17/−16」口径无法从 records 复现——records 最新 08-08、无校正条目）。**✅ 2026-08-19 专项校正场销账（Doctor 裁双销账）**：现核 records 08-15 周更场（挂账时「records 最新 08-08」系早场快照，当晚周更场已覆盖）——奥比 Δ+4.5（ds 74.0/claude 69.5 · ds 证据逐条对 canonical company_Orbbec properties 全命中 · ds 84→74 收敛）、海光 Δ−8.5（claude 60.0/ds 51.5 · 真分歧共存）；均低于 |Δ|≥15 阈值制度出队（08-08 起），主分不变（69.5/60.0）。海光 ds 供需端「中芯产能」错链锚登记 龙鱼五力 GOTCHAS NOTE-20260819-001。留痕：决策记录 2026-08-19 条 + 对比校正/全库_对比_20260815.md 追记。

- [x] **跨项目 · .bak_20260808 批次清理（2026-08-08 挂 · dated 08-15 后 · 2026-08-22 勾销迁档 · 白泽 3 件尾清）**：当日改动回滚备份留 7 天——三连场 6 件（各仓 .bak_20260808）＋白泽大宗 3 件（GOTCHAS.md / lib_public_read.py / weekly_health.py）＋**渊图 08-08 三轮入库产物（Doctor 08-09 批搭本车）**：`mapping/` 下 `行业知识图谱_v2_20260808_{231709_5篇,231813_6篇,234329_5篇,234455_6篇}.json` 四件合并中间图（含 23:38 毒批，gitignore 已盖住未进仓）＋其 pre_recheck/region .bak 六件（212837/231813/234455 三组各二）＋根目录 `index.json.bak_20260808_hold6`。08-15 后随 /todo 统一清；清前确认对应改动均已稳定（含明早班首跑）。**2026-08-15 开批（Doctor 裁）**：20 件今清（渊图 14 含漏点 2 + brain 3 + 海螺姑娘 3 · tar→fuxi 冷归档）；白泽大宗 3 件顺延明早 08-16 01:09 定时首验班后清；Market-Data 2 件留到「切 volume_trillion」条执行时处置（注记已挂该条）。**2026-08-15 收口**：18 件 fuxi 冷归档完整（双端 SHA256 bc8529b8… 一致）+ 212837 region 残件已删 + index hold6 已 git rm（commit `9a772dc` 已 push）；`234455_6篇.json.bak_20260808_region`（11.2MB）丢失留痕（region 打标备份·主体图已归档 fuxi·打标可从 canonical 重跑·影响低）；白泽 3 件与 Market-Data 2 件维持原裁定。
  **✅ 2026-08-22 白泽 3 件尾清（实跑证据）**：前置已过（08-16 01:09 定时首验班 commit「白泽: 08-16周日班数据产物(_health 01:23 overall ok…)」在链）；find 实盘 3 件与台账口径一致（GOTCHAS.md.bak_20260808 / scripts/lib_public_read.py.bak_20260808 / scripts/reports/weekly_health.py.bak_20260808，全仓 *.bak_20260808=3）；tar 打包 `bak_20260808_baize3.tar.gz`（22,946B·3 件）→ scp F:\Mac_Quarantine\ → 双端 SHA256 `d7af4e7b…F00666F` 逐位一致 → 删本地源（rm exit 0 · 全仓 *.bak_20260808 复扫 = 0）。Market-Data 2 件注记已挂「切 volume_trillion」条，本条闭环。
