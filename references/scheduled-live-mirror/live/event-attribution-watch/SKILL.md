...
7. 产出三件：
   ...
   **视觉规范 v2（2026-08-03 Doctor 批 · 警示页原型层语法上岗）**：
   - 页首：免责短版并入副题下一行（「解释层台账 · nowcast 同步仪表 · 非预警器 · 永不直接驱动仓位或温度」·§06 缩并形态）；metabar chips 用语义色（alert=残差告警/新提案 · warn=源分歧/待裁 · good=解冻/通道恢复）。
   - **残差告警卡**（页头下·通栏）：残差 >0.3pp 阈值连续 ≥2 日时渲染——左「杀伤」标签＋五段灯（亮数按残差分档：>0.3=1 · >0.6=2 · >1.0=3 · >2.0=4 · >3.0=5，灯位固定色绿绿黄橙红），右时间语义独立配色（连续第 N 日＝已触发红）；卡内一句动态结论（缺口构成：哪个未批/新提案形态）。残差回落阈内即撤卡。
   - **提案区优先级**：触及注册表首例基线/量级带原始数的提案（当前=P-16）单独渲染为优先卡——左上石刻「危」浮层（字酷堂石刻体·呼吸·prefers-reduced-motion 降级静态发光）＋杀伤灯＋时间语义（待裁状态）；其余提案保持表格。新提案若有同级性质，同法渲染。
