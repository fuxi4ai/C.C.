...
7. 产出三件：
   ...
   **视觉规范 v2.4（2026-08-03 Doctor 批 · 警示页原型层语法上岗）**：
   ...
   - **折叠架构（⑩铁律 · v2.4 扩到节级）**：**第一节归因曲线常显（首节见图）；第二~五节全部默认折叠**（注册表/记账表/提案区/变更记录），每节一张 details 折叠卡，summary 行必须含该节最新动态结论（禁止只写标题）；第三节卡内各复核小节（分时复核/截面签名/源订正/证据腿/恒等式对账/慢变量序列）同样「结论常显一行＋details 折叠正文」。
   - **提案区清空规则（v2.4 增）**：已获批的提案**从提案区移除**（批后移入变更记录；逐条原文迁入台账 §七附·已裁提案档案），提案区只留在途条目；全部裁完时渲染「当前无在途提案」一行+铁律 callout，不留已完成条目占位。
