...
7. 产出三件：
   ...
   **视觉规范 v2.9（2026-08-03 Doctor 批 · 警示页原型层语法上岗）**：
   ...
   - **折叠架构（⑩铁律 · 节级）**：**第一节归因曲线常显（首节见图）；第二~六节全部默认折叠**（注册表/记账表/提案区/方程迭代记录/验证日历），每节一张 details 折叠卡；**节标题只留节名本身、不带括号附注**（版本/纪律/编号说明等信息放简介行或正文·v2.8）；**summary 行＝该节是什么＋最新动态，一律用易懂文字**（读者不查术语表也能懂；先说是什么、再说最新状态，禁止只写标题或堆缩写）；第三节卡内各复核小节（分时复核/截面签名/源订正/证据因子/恒等式对账/慢变量序列）同样「结论常显一行＋details 折叠正文」。
   - **遗漏因子事后分析卡**（挂第一节末·默认折叠）：残差 >0.3pp 连续 ≥2 日时，追加一张「遗漏因子事后分析」卡——事后 sizing 分摊缺口构成（各因子份额带区间），必标「事后 sizing 非记账 · G-04 高危 · 不可加总成对账表 · 历史残差不回填」。
   ...
   - **措辞纪律（v2.5 增 · 语域规范「压缩造词」条）**：禁自造压缩新词（如「两新腿」），用完整说法（「两条新的解释因子」「两个新自变量」）；**层级命名**：因子（注册级）· 分项（签名构件，如油价分项/VIX 分项/传染分项）· 段（日期段，如 07-29 段）· 项（集合元素）——**禁新造「X腿」**；当前注册名：**退款因子 / 杂质因子 / 证据因子**（原称…腿·2026-08-03 Doctor 改今名）。
