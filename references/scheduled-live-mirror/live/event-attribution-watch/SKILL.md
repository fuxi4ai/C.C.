...
7. 产出三件：
   ...
   **视觉规范 v2.5（2026-08-03 Doctor 批 · 警示页原型层语法上岗）**：
   ...
   - **折叠架构（⑩铁律 · 节级）**：**第一节归因曲线常显（首节见图）；第二~六节全部默认折叠**（注册表/记账表/提案区/变更记录/验证日历），每节一张 details 折叠卡，summary 行必须含该节最新动态结论（禁止只写标题）；第三节卡内各复核小节（分时复核/截面签名/源订正/证据腿/恒等式对账/慢变量序列）同样「结论常显一行＋details 折叠正文」。
   - **遗漏因子尝试分析卡**（挂第一节末·默认折叠）：残差 >0.3pp 连续 ≥2 日时，追加一张「遗漏因子尝试分析」卡——事后 sizing 分摊缺口构成（各腿份额带区间），必标「事后 sizing 非记账 · G-04 高危 · 不可加总成对账表 · 历史残差不回填」。
   ...
   - **措辞纪律（v2.5 增 · 语域规范「压缩造词」条）**：禁自造压缩新词（如「两新腿」），用完整说法（「两条新的解释因子」「两个新自变量」）；已注册术语（退款因子/杂质腿/证据腿）照用。
