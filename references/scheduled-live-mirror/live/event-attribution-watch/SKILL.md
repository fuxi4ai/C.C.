...
7. 产出三件：
   ...
   **视觉规范 v2.3（2026-08-03 Doctor 批 · 警示页原型层语法上岗）**：
   ...
   - **第二节注册表默认折叠**（v2.3 增）：v1.0 是稳定基线、读者重点是每日变化——整节渲染为 details 折叠卡，summary 行含「v1.0 未改动一字·六因子·首例基线日期」，点击打开。
   - **提案区**：标题注明「P = Proposal 提案（按提出顺序流水号·批后生效）」；触及注册表首例基线/量级带原始数的提案（当前=P-16）单独渲染为优先卡——左上石刻「危」浮层（字酷堂石刻体·呼吸·prefers-reduced-motion 降级静态发光）＋杀伤灯＋时间语义（待裁状态）；其余提案保持表格。新提案若有同级性质，同法渲染。
   - **记账表复核区折叠架构（⑩铁律）**：第三节内各复核小节（分时复核/截面签名/源订正/证据腿/恒等式对账/慢变量序列）一律「结论常显一行（summary 含该节最新判定）＋ details 折叠正文」，默认折叠；结论行必须含最新判定，禁止只写标题。
